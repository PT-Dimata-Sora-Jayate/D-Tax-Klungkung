...
public class PstPaymentPbbIprotax extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_PAYMENTPBBIPROTAX = "IPROTAXPBB.PEMBAYARAN_SPPT";
    ...
    //2017-132
    public static final int FLD_JMLPENGURANGANPOKOK = 23;
    public static final int FLD_JMLPENGURANGANDENDA = 24;
    ...
    public static synchronized long insertExc(PaymentPbbIprotax entPaymentPbbIprotax) throws DBException {
        try {
            ...
            pstPaymentPbbIprotax.setString(FLD_DATAPEMBAYARAN, entPaymentPbbIprotax.getDataPembayaran());
            //2017-132
            pstPaymentPbbIprotax.setDouble(FLD_JMLPENGURANGANPOKOK, entPaymentPbbIprotax.getJmlpenguranganpokok());
            pstPaymentPbbIprotax.setDouble(FLD_JMLPENGURANGANDENDA, entPaymentPbbIprotax.getJmlpengurangandenda());
			} catch (Exception exc){
				System.out.println(exc.toString());
			}
            
            pstPaymentPbbIprotax.insert();
			System.out.println(pstPaymentPbbIprotax.getInsertSQL());
	...
	public static Vector listPaymentPBBDaily(int limitStart, int recordToGet, String whereClause, String order, String group) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
			String sql = "";
			if (AppSetting.SQL_VERSION == AppSetting.DBSVR_ORACLE){
				sql = "SELECT TO_DATE(TO_CHAR(TGL_PEMBAYARAN_SPPT,'DD Month YYYY'),'DD MM YYYY') TGL_PEMBAYARAN_SPPT, SUM(JML_PBB_YG_DIBAYAR) AS JML_PBB_YG_DIBAYAR,"
						+ "SUM(JML_DENDA_SPPT) AS JML_DENDA_SPPT FROM VIEW_PEMBAYARAN_PBB";
				if (whereClause != null && whereClause.length() > 0) {
					sql = sql + " WHERE " + whereClause;
				}
				...
				sql = "SELECT CAST(TGL_PEMBAYARAN_SPPT AS DATE) TGL_PEMBAYARAN_SPPT, SUM(JML_PBB_YG_DIBAYAR) AS JML_PBB_YG_DIBAYAR,"
						+ "SUM(JML_DENDA_SPPT) AS JML_DENDA_SPPT FROM VIEW_PEMBAYARAN_PBB";
				if (whereClause != null && whereClause.length() > 0) {
					sql = sql + " WHERE " + whereClause;
				}
            ...
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                PaymentPbbIprotax entPaymentPbb = new PaymentPbbIprotax();
				entPaymentPbb.setTglPembayaranSppt(rs.getDate(1));
				entPaymentPbb.setJmlPbbYgDibayar(rs.getDouble(2));
				entPaymentPbb.setJmlDendaSppt(rs.getDouble(3));
	...
	public static Vector listPerBulan(int limitStart, int recordToGet, String whereClause, String order, String groupBy) {
        ...
        try {
			String sql = "";
			if (AppSetting.SQL_VERSION == AppSetting.DBSVR_ORACLE){
				sql = "SELECT SUM("+fieldNames[FLD_JMLPBBYGDIBAYAR]+") "+fieldNames[FLD_JMLPBBYGDIBAYAR]+", "
						+ "SUM("+fieldNames[FLD_JMLDENDASPPT]+") "+fieldNames[FLD_JMLDENDASPPT]+", "
						+ "TO_DATE(TO_CHAR("+fieldNames[FLD_TGLPEMBAYARANSPPT]+",'Month YYYY'),'MM YYYY') "+fieldNames[FLD_TGLPEMBAYARANSPPT]+" FROM VIEW_PEMBAYARAN_PBB";
				if (whereClause != null && whereClause.length() > 0) {
					sql = sql + " WHERE " + whereClause;
				}
				...
				sql = "SELECT SUM("+fieldNames[FLD_JMLPBBYGDIBAYAR]+") "+fieldNames[FLD_JMLPBBYGDIBAYAR]+", "
						+ "SUM("+fieldNames[FLD_JMLDENDASPPT]+") "+fieldNames[FLD_JMLDENDASPPT]+", "
						+ "CONCAT(convert(varchar(7), TGL_PEMBAYARAN_SPPT, 126),'-01') AS "+fieldNames[FLD_TGLPEMBAYARANSPPT]+" FROM VIEW_PEMBAYARAN_PBB";
				if (whereClause != null && whereClause.length() > 0) {
					sql = sql + " WHERE " + whereClause;
				}
            ...
            while (rs.next()) {
                PaymentPbbIprotax entPaymentPbb = new PaymentPbbIprotax();
                entPaymentPbb.setTglPembayaranSppt(rs.getDate(3));
				entPaymentPbb.setJmlPbbYgDibayar(rs.getDouble(1));
				entPaymentPbb.setJmlDendaSppt(rs.getDouble(2));
                lists.add(entPaymentPbb);
            }
            rs.close();
            return lists;
