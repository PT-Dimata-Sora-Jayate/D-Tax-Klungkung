   ...
   public static String[] fieldNames = {
    ...
    public static void resultToObjectIpProtax(ResultSet rs, PaymentPbb entPaymentPbb) {
        try {
			  ...
			  entPaymentPbb.setDendaSppt(rs.getDouble("JML_DENDA_SPPT"));
              entPaymentPbb.setStatus(0);
			  entPaymentPbb.setPembayaranSpptKe(rs.getInt("PEMBAYARAN_SPPT_KE"));
			  entPaymentPbb.setTglPembayaranSppt(rs.getDate("TGL_PEMBAYARAN_SPPT"));
//            entPaymentPbb.setKdPropinsi(rs.getString(PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_KDPROPINSI]));
//            entPaymentPbb.setKdDati2(rs.getString(PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_KDDATI2]));
//            entPaymentPbb.setKdKecamata(rs.getString(PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_KDKECAMATA]));
//            entPaymentPbb.setKdKelurahan(rs.getString(PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_KDKELURAHAN]));
...
//            entPaymentPbb.setNipRekamByrSppt(rs.getString(PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_NIPREKAMBYRSPPT]));
	...
	public static Hashtable<String, Vector> hashIpprotax(int limitStart, int recordToGet, String whereClause, String order) {
        Hashtable hashtable = new Hashtable();
		Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "select VIEW_PEMBAYARAN_PBB.* from VIEW_PEMBAYARAN_PBB INNER JOIN VIEW_PBB ON VIEW_PEMBAYARAN_PBB.NOP = VIEW_PBB.NOP AND VIEW_PEMBAYARAN_PBB.THN_PAJAK_SPPT = VIEW_PBB.THN_PAJAK_SPPT";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + whereClause;
            }
            if (order != null && order.length() > 0) {
    ...
    public static double getTotalPembayaran(String whereClause) {
        double total = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT NOP, SUM(ISNULL(JML_PBB_YG_DIBAYAR, 0) - ISNULL(JML_DENDA_SPPT, 0)) AS PEMBAYARAN, "
					+ " THN_PAJAK_SPPT FROM VIEW_PEMBAYARAN_PBB"
					+ " WHERE "+whereClause+" GROUP BY NOP, THN_PAJAK_SPPT";
            dbrs = DBHandler.execQueryResult(sql);
			System.out.println(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
