...
public class AjaxBankGaransi extends HttpServlet {
    ...
    public String showForm2(HttpServletRequest request){
        ...
        BankGaransi bankGaransi = new BankGaransi();
        BankGaransi bankGaransiValidasi = new BankGaransi();
        ...
        if (oid != 0) {
            try {
                bankGaransiValidasi = PstBankGaransi.fetchExc(oid);
                bankGaransi = (BankGaransi) bankGaransi.getValidasiDetail(bankGaransiValidasi);
            } catch (Exception e) {
                String returnData = "alert('Terjadi Kesalahan "+e.getStackTrace()+"')";
                e.printStackTrace();
            }
            
            if(bankGaransi.getErrorNominal().equals("")){
                sNominal = Formater.formatNumber(bankGaransi.getNominal(),"###,###,##0");
            }
	...
	+ "<div class='row'>"
		...
		+ "<div class='form-horizontal' style='padding:10px;background:#fff;border:5px solid #f1f1f1;'>"
			...
			+ "<div class='col-sm-8'>"
			    + "<div class='input-group'>"
				+ "<input type='text' "+bankGaransi.getValidasiTglKondisi()+" class='form-control datepicker' name='"+ FrmBankGaransi.fieldNames[FrmBankGaransi.FRM_FIELD_TGL_KONDISI] +"' id='"+ FrmBankGaransi.fieldNames[FrmBankGaransi.FRM_FIELD_TGL_KONDISI] +"' value='"+tglKondisi+"' data-type='text' data-error-message='"+FrmBankGaransi.fieldError[FrmBankGaransi.FRM_FIELD_KODE_KONDISI]+"'>"
				+ "<div class='input-group-addon'>"
				    + "<i class='fa fa-calendar'></i>"
				+ "</div>"
			    + "</div>"
			+"</div><i style='color:red'>"+bankGaransi.getErrorTglKondisi()+"</i>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
