...
public class AjaxPengurusPemilik extends HttpServlet {
    ...
    private JSONArray jSONArrayPengurusPemilik = new JSONArray();
	...
	this.jSONObject = new JSONObject();
        this.jSONArrayPengurusPemilik = new JSONArray();
	...
	try{
	    ...
	    this.jSONObject.put("RETURN_DATA_PENGURUS", this.jSONArrayPengurusPemilik);
	...
	if(this.dataFor.equals("showform")){
        ...
        }else if(this.dataFor.equals("loadsummarypangsa")){    
             this.htmlReturn = getSummaryPangsa(request);
        }else if(this.dataFor.equals("showotherform")){
            this.htmlReturn = otherForm(request);
        }else if(this.dataFor.equals("showlistpenguruspemilikpangsa")){
            this.htmlReturn = showListForm(request);
        }else if(this.dataFor.equals("selectidentitas")){
            getDataPengurusPemilik(request);
        }
    }
	...
	if(this.dataFor.equals("listdatasearch")){
	    ...
	    jSONObject = listDataTables(request, response, cols,"listdatapengurus", this.jSONObject);
            ...
            String[] cols = { 
                ""+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PENGURUS_ATAU_PEMILIK_OID]+"",
                ""+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NO_IDENTITAS]+"",
                ""+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NAMA_PENGURUS]+"",
                ""+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_ALAMAT]+"",
	...
	String cif = FRMQueryString.requestString(request, "cif");
        ...
        if(userGroup != 4){
            if(whereClause.length() > 0){
                ...
                whereClause += ""+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID]+"='"+periodeId+"'";
            }else{
        ...
        }else if(dataFor.equals("showlistpenguruspemilikpangsa")){
	    if(whereClause.length() > 0){
		whereClause += " AND (pengurus."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+" LIKE '%"+this.searchTerm+"%' "
                        + "OR pengurus."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NAMA_PENGURUS]+" LIKE '%"+this.searchTerm+"%' "
                        + "OR pengurus."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_ALAMAT]+" LIKE '%"+this.searchTerm+"%')";
	    ...
	    if(whereClause.length() > 0){
		whereClause += ""
                        + " AND ("+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NO_IDENTITAS]+" LIKE '%"+this.searchTerm+"%' )";
	    }else{
		whereClause += ""
                        + " ("+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NO_IDENTITAS]+" LIKE '%"+this.searchTerm+"%' )";
	    }
        }
	...
	if(dataFor.equals("listdatasearch") || dataFor.equals("listdatapengurus") || dataFor.equals("showlistpenguruspemilik")  || this.dataFor.equals("listdatapengurushome")){
	    total = PstPengurusAtauPemilik.getCountPengurusPemilik(whereClause,periodeId,kodeCabang);
	}
        
        if(dataFor.equals("showlistpenguruspemilikpangsa")){
            total = PstPengurusAtauPemilik.getCountPangsaKurangLebihSeratus(whereClause, periodeId, kodeCabang);
        }
        
        if(dataFor.equals("searchidentitas")){
            total = PstPengurusAtauPemilik.getCount(whereClause);
        }
...
//                }
		...
		if(whereClause.length() > 0){
            ...
            }else if (dataforx.equals("listdatapengurus") || dataforx.equals("listdatapengurushome")){
                String cifParam = FRMQueryString.requestString(request, "FRM_FIELD_CIF");
                whereClause  = " "
                    + " 1=1"
                    + " AND pengurus."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+" = '"+cifParam+"'"
	...
	if(dataforx.equals("listdatasearch") || dataforx.equals("listdatapengurus") || dataforx.equals("showlistpenguruspemilik") || this.dataFor.equals("listdatapengurushome")){
            listData = PstPengurusAtauPemilik.getlistJoin(start, amount, order, whereClause, periodeId, kodeCabang );
        }
        
        if(dataFor.equals("showlistpenguruspemilikpangsa")){
            listData = PstPengurusAtauPemilik.getlistJoinPangsa(start, amount, order, whereClause, periodeId, kodeCabang ); 
        }
        
        if(dataFor.equals("searchidentitas")){
            listData = PstPengurusAtauPemilik.list(start, amount, whereClause, order);
        }
		...
		array.put(ja);
                ...
                ja.put(""+(pengurusAtauPemilik.getCif()));
                ja.put(""+(pengurusAtauPemilik.getPangsaKepemilikan()));
		...
		array.put(ja);
	    }else if(dataforx.equals("showlistpenguruspemilikpangsa")){
		pengurusAtauPemilik = (PengurusAtauPemilik) listData.get(i);
		ja.put(""+(this.start+i+1));
                ja.put(""+pengurusAtauPemilik.getCif());
                ja.put(""+(pengurusAtauPemilik.getNamaPengurus()== null ? "-" : pengurusAtauPemilik.getNamaPengurus()));
		...
		array.put(ja);
	    } else if(dataforx.equals("searchidentitas")){
		pengurusAtauPemilik = (PengurusAtauPemilik) listData.get(i);
		ja.put(""+(this.start+i+1));
                ja.put(""+pengurusAtauPemilik.getNoIdentitas());
                ja.put(""+(pengurusAtauPemilik.getNamaPengurus()== null ? "-" : pengurusAtauPemilik.getNamaPengurus()));
...
//                    buttonUpdate += "<a href='"+approot+"/dslik/perlengkapandata/perlengkapandata.jsp?command=8&oid="+pengurusAtauPemilik.getDebiturOid()+"&type="+pengurusAtauPemilik.getDebiturType()+"&cif="+pengurusAtauPemilik.getCif()+"&activetab=AjaxPengurusPemilik&otheroid="+pengurusAtauPemilik.getOID()+"' class='btn btn-warning'><i class='fa fa-pencil'></i> View</a>";
//                }
		ja.put("<button type='button' class='btn btn-sm btn-success btn-selectnoidentitas' data-oid='"+pengurusAtauPemilik.getOID()+"' data-for='selectidentitas' data-command='"+Command.NONE+"' data-target='AjaxPengurusPemilik'><i class='fa fa-check'></i> Select</button>");
    ...
    public String showForm(HttpServletRequest request){
        ...
        String cifDua = FRMQueryString.requestString(request, "cif");
        double pangsaSum=0;
        if (cif.length()>0){
        ...
        if (cifDua.length()>0){
            ...
            try{
                ...
                if(listPeriode != null){
                    periode = (Periode) listPeriode.get(0);
                    periodeId=periode.getOID();
                    String wherePangsa = PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+"='"+cif+"' AND "+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID]+"='"+periodeId+"'";
                    pangsaSum = PstPengurusAtauPemilik.getSumPangsa(wherePangsa);
                }
            }catch(Exception ex){
                            ...
                            +"</div>"
                             +"<div class='box-header'>";
                                ...
                                if(privDelete){
                                    ...
                                    returnData +=" <div text-right><b>Total Pangsa : "+pangsaSum+"</b><div>";
                            returnData+=""
                            +"</div>"
                        +"</div>"
                +"</div>"
    ...
    public String showForm2(HttpServletRequest request){
        ...
        double pangsaSum =0;
        String textPangsa="";
        if(!pengurus.getCif().equals("") ){
            String wherePangsa = PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+"='"+pengurus.getCif()+"' "
                    + "AND "+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID]+"='"+oidPeriodeWrite+"' "
                    + "AND KODE_JABATAN in ('01','02','03','04','05','06','07','08','09','10','11','12','13','19') "
                    + "AND dslik_pengurus_atau_pemilik.STATUS_PENGURUS=1";
            pangsaSum = PstPengurusAtauPemilik.getSumPangsa(wherePangsa);
            if(pangsaSum>100){
                textPangsa="Jumlah pangsa kepemilikan jabatan pemilik untuk CIF "+pengurus.getCif()+" harus 100 persen. Jumlah saat ini "+pangsaSum;
            }
            if(pangsaSum<100){
                textPangsa="Jumlah pangsa kepemilikan jabatan pemilik untuk CIF "+pengurus.getCif()+" harus 100 persen. Jumlah saat ini "+pangsaSum;
            }
	...
	+ "<div class='row'>"
		...
		+ "<div class='form-horizontal' style='padding:10px;background:#fff;border:5px solid #f1f1f1;'>"
			...
			+ "<div class='col-sm-6'>";
                            ...
                            returnData += "</div>"
                        + "<div class='col-sm-2'>"
                            + "<button type='button' class='btn btn-primary' id='btn-showmodalnoidentitas' data-for='searchidentitas' data-command='"+Command.NONE+"' data-target='AjaxPengurusPemilik'><i class='fa fa-search'></i></button>"
                        + "</div>"
...
//			    + "<input type='text' class='form-control' name='jenisidentitas'>"
                            Vector vCdJenisId =  PstContentDataJenisIdentitas.listJenisIdentitasPengurusWithoutOid(0,0,"","");
...
//			+ "</div>"
			...
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='"+ FrmPengurusAtauPemilik.fieldNames[FrmPengurusAtauPemilik.FRM_FIELD_PANGSA_KEPEMILIKAN] +"' id='"+ FrmPengurusAtauPemilik.fieldNames[FrmPengurusAtauPemilik.FRM_FIELD_PANGSA_KEPEMILIKAN] +"' value='"+pengurus.getPangsaKepemilikan()+"' data-required='required' data-required='required' data-number='true' data-alpha='false' data-special='.' data-type='text' data-error-message='"+FrmPengurusAtauPemilik.fieldError[FrmPengurusAtauPemilik.FRM_FIELD_PANGSA_KEPEMILIKAN]+"'>"
                                    + "<br><i style='color:red;'>Jika nilainya decimal, gunakan . (titik)</i></label>"
			+ "</div>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
		    ...
		    + "</div>"
                    ...
                    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
                            + "<i class='fa fa-question-circle' data-toggle='tooltip' data-placement='right' title=''></i> "
                            + "Total Pangsa Kepemilikan  <i style='color:red;'>(m*</i></label>"
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='' id='' value='"+pangsaSum+"' readonly='readonly' data-required='required' data-required='required' data-number='true' data-alpha='false' data-special=',' data-type='text' data-error-message=''>"
                                    + "<br><i style='color:red;'>"+textPangsa+"</i></label>"
			+ "</div>"
		    + "</div>"  
    ...
    public void getDataPengurusPemilik(HttpServletRequest request) {
        PengurusAtauPemilik pengurus = new PengurusAtauPemilik();
        try {
            pengurus = PstPengurusAtauPemilik.fetchExc(oid);
        } catch (Exception e) {
        ...
        try {
            ...
            jsonObject.put("no_identitas", "" + pengurus.getNoIdentitas());
            jsonObject.put("jenis_identitas", "" + pengurus.getJenisIdentitas());
            jsonObject.put("nama_pengurus", "" + pengurus.getNamaPengurus());
            jsonObject.put("jenis_kelamin", "" + pengurus.getJenisKelamin());
            jsonObject.put("alamat", "" + pengurus.getAlamat());
            jsonObject.put("kelurahan", "" + pengurus.getKelurahan());
            jsonObject.put("kecamatan", "" + pengurus.getKecamatan());
            jsonObject.put("kodekab", "" + pengurus.getKodeKabupaten().trim());
            this.jSONArrayPengurusPemilik.put(jsonObject);
    ...
    public String getSummary(HttpServletRequest request){
        ...
        int summary = 0;
        if(this.dataFor.equals("loadsummary") || this.dataFor.equals("loadsummarynull")){
            summary = PstPengurusAtauPemilik.getCountPengurusPemilik(whereClause, periodeId, kodeCabang);
        }else{
            summary = SessionProsesPerlengkapan.getCountJoinPengurusPemilik(whereClause, periodeId);
        }
        returnData +=""+summary;
        return returnData;
    }
    ...
    public String getSummaryPangsa(HttpServletRequest request){
        String returnData = "";
        int userGroup = FRMQueryString.requestInt(request, "usergroup");
        String kodeCabang = FRMQueryString.requestString(request, "kodecabang");
        long periodeId = FRMQueryString.requestLong(request, "periode");
        ...
        whereClause = " pengurus."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID]+"='"+periodeId+"' ";
        int summary = 0;
        summary = PstPengurusAtauPemilik.getCountPangsaKurangLebihSeratus(whereClause, periodeId, kodeCabang);
        returnData +=""+summary;
        return returnData;
    }
    ...
    public String otherForm(HttpServletRequest request){
        ...
        Vector listData = SessionProsesPerlengkapan.listOtherPengurusPemilik(0, 0, whereClause, "l."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+" ASC",periodeId);
        returnData+=""
