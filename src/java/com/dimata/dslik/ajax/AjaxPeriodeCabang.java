...
public class AjaxPeriodeCabang extends HttpServlet {
    ...
    public JSONObject getData(int total, HttpServletRequest request, String datafor, long periodeId) {
        ...
        for (int i = 0; i <= listData.size() - 1; i++) {
            ...
            if (datafor.equals("list")) {
                ...
                if(dataLengkap(periodeCabang.getOID())){
                    app=true;
                    message="";
                }
                
                if(!privApprove){
                    message += "Tidak punya hak approve";
                }
    ...
    private synchronized  boolean dataLengkap(long periodeCabangId){
        ...
        if(count > 0){
            message += "Fasilitas Lainnya : "+count;
            return false;
        }else{
            isCorrect = true;
        }
        ...
        count = PstPenjamin.getCountJoinx(whereClause);
        if(count > 0){
             message += "Penjamin : "+count;
            return false;
        }else{
            isCorrect = true;
        }
