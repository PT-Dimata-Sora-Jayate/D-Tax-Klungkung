...
public class AjaxLaporanKeuanganDebitur extends HttpServlet {
   ...
   //DATATABLES
	...
	if(this.dataFor.equals("showform")){
        ...
        }else if(this.dataFor.equals("showlistlaporankeuangandebiturWajib")){
            this.htmlReturn = showListFormLaporanKeuanganWajibDiInput(request);    
        ...
        }else if(this.dataFor.equals("loadsummarylaporankeuanganwajib")){
            this.htmlReturn = getSummaryLaporanKeuanganWajib(request);
        }
    }
        ...
        if(userGroup != 4){
            if(whereClause.length() > 0){
                ...
                if(!kodeCabang.equals("")){
                    whereClause += " laporan."+PstLaporanKeuanganDebitur.fieldNames[PstLaporanKeuanganDebitur.FLD_PERIODE_ID]+"='"+periodeId+"'";
                }else{
                    whereClause += " laporan."+PstLaporanKeuanganDebitur.fieldNames[PstLaporanKeuanganDebitur.FLD_PERIODE_ID]+"='"+periodeId+"'";
                }
            }
	...
	if(dataFor.equals("listdatasearch") || dataFor.equals("listdatalaporan") || this.dataFor.equals("listdatalaporanhome")){
	    total = PstLaporanKeuanganDebitur.getCountJoin(whereClause,periodeId,kodeCabang);
	}else if(dataFor.equals("showlistlaporankeuangandebitur")){
            total = PstLaporanKeuanganDebitur.getCountJoin(whereClause,periodeId,kodeCabang);
        }else if(dataFor.equals("showlistlaporankeuangandebiturWajib")){
            total = SessionProsesPerlengkapan.getCountJoinLaporanKeuanganWajib(periodeId,kodeCabang);
        }
	...
	if(datafor.equals("listdatasearch") || datafor.equals("listdatalaporan") || this.dataFor.equals("listdatalaporanhome")){
	    listData = PstLaporanKeuanganDebitur.getlistJoin(start, amount, whereClause, order, periodeId,kodeCabang);
	}else if(datafor.equals("showlistlaporankeuangandebitur")){
	    listData = PstLaporanKeuanganDebitur.getlistJoin(start, amount, whereClause, order, periodeId,kodeCabang);
	}else if(dataFor.equals("showlistlaporankeuangandebiturWajib")){
            listData = SessionProsesPerlengkapan.getListJoinJoinLaporanKeuanganWajib(periodeId,kodeCabang);
        }
		...
		ja.put(""+(this.start+i+1));
                ...
                Debitur debitur = (Debitur) listData.get(i);
		ja.put(""+(this.start+i+1));
                ja.put(""+(debitur.getCif()));
                if(privView){
                    if(debitur.getKodeJenisNsb()==1){
    ...
    public String showListFormLaporanKeuanganWajibDiInput(HttpServletRequest request){
        String returnData = "";
        returnData+=""
        + "<div id='showListElement'>"
            + "<table class='table table-bordered table-striped'>"
    ...
    public String getSummary(HttpServletRequest request){
        ...
        int summary = PstLaporanKeuanganDebitur.getCountJoin(whereClause,periodeId,kodeCabang); 
    ...
    public String getSummaryLaporanKeuanganWajib(HttpServletRequest request){
        String returnData = "";
        int userGroup = FRMQueryString.requestInt(request, "usergroup");
        String kodeCabang = FRMQueryString.requestString(request, "kodecabang");
        ...
        int summary = SessionProsesPerlengkapan.getCountJoinLaporanKeuanganWajib(periodeId, kodeCabang); 
