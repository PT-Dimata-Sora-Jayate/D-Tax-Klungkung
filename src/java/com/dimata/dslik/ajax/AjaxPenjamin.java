...
public class AjaxPenjamin extends HttpServlet {//DATATABLES
    ...
    private JSONArray jSONArrayPenjamin = new JSONArray();
	...
	this.jSONObject = new JSONObject();
        this.jSONArrayPenjamin = new JSONArray();
	...
	try{
	    ...
	    this.jSONObject.put("FRM_FIELD_DATE_END", this.dateEnd);
            this.jSONObject.put("RETURN_DATA_PENJAMIN", this.jSONArrayPenjamin);
	...
	if(this.dataFor.equals("showform")){
        ...
        }else if(this.dataFor.equals("selectidentitas")){
            getDataPenjamin(request);
        }
    }
	...
	if(this.dataFor.equals("listdatasearch")){
	    ...
	    jSONObject = listDataTables(request, response, cols, this.dataFor, this.jSONObject);
	}else if (this.dataFor.equals("searchidentitas")){
            String[] cols = { 
                PstPenjamin.fieldNames[PstPenjamin.FLD_PENJAMIN_OID],
                PstPenjamin.fieldNames[PstPenjamin.FLD_CIF],
                PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN],
	    ...
	    jSONObject = listDataTables(request, response, cols,"searchidentitas", this.jSONObject);
        }else if (this.dataFor.equals("showform2")){
            String[] cols = { 
                PstPenjamin.fieldNames[PstPenjamin.FLD_PENJAMIN_OID],
                PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN],
	...
	// penetapan periode id
        ...
        if(dataFor.equals("listdatasearch") || dataFor.equals("showlistpenjamin") || dataFor.equals("searchidentitas")){
            alias = "penjamin.";
        }
        ...
        if(userGroup != 4){
            ...
            }else if (dataFor.equals("searchidentitas")){
                whereClause += " "+alias+""+PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID]+"='"+periodeId+"'";
            }else{
		...
		whereClause += " (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_CIF]+" LIKE '%"+this.searchTerm+"%' "
            ...
            //whereClause = "1=1";
            whereClause = "penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID]+"='"+periodeId+"'";
            String cifParam = FRMQueryString.requestString(request, "FRM_FIELD_CIF");
            whereClause += ""
                + " AND penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_CIF]+"='"+cifParam+"'"
                + " AND (" 
                    + " penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" like '%"+this.searchTerm+"%'"
                    + " OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_REKENING]+" like '%"+this.searchTerm+"%'"
                    + " OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_JENIS_IDENTITAS]+" like '%"+this.searchTerm+"%'"
                    + " OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NAMA_IDENTITAS]+" like '%"+this.searchTerm+"%'"
                    + " OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_KODE_GOL_PENJAMIN]+" like '%"+this.searchTerm+"%'"
                    + " OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_ALAMAT_PENJAMIN]+" like '%"+this.searchTerm+"%'"
                + ")";
	    ...
	    if(whereClause.length() > 0){
		whereClause += ""
                        + " AND (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' )";
	    }else{
		whereClause += ""
                        + " (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' )";
	    }
        }else if (dataFor.equals("showform2")){
            //whereClause = "1=1";
            whereClause = "penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID]+"='"+periodeId+"'";
            String cifParam = FRMQueryString.requestString(request, "FRM_FIELD_CIF");
            whereClause += ""
	...
	if(dataFor.equals("listdatasearch")){
	    total = PstPenjamin.getCountJoin(whereClause,periodeId,kodeCabang);
	}else if (dataFor.equals("listdatapenjamin") || this.dataFor.equals("listdatapenjaminhome")){
            total = PstPenjamin.getCountJoin(whereClause,periodeId,kodeCabang);
        }else if(dataFor.equals("showlistpenjamin")){
            total = PstPenjamin.getCountJoin(whereClause,periodeId,kodeCabang);
        }else if(dataFor.equals("searchidentitas")){
            total = PstPenjamin.getCountJoin(whereClause,periodeId,"");
        }else if (dataFor.equals("showform2")){
            total = PstPenjamin.getCountJoin(whereClause,periodeId,kodeCabang);
        }
	...
	String cif = FRMQueryString.requestString(request, "cif");
	    ...
	    if(whereClause.length() > 0){
                if (dataFor.equals("listdatapenjamin") || this.dataFor.equals("listdatapenjaminhome")){
                    ...
                    //whereClause += " AND ("+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' )";
                }else{
                    whereClause += " AND (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_CIF]+" LIKE '%"+this.searchTerm+"%' "
                        + "OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' "
                        + "OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_ALAMAT_PENJAMIN]+" LIKE '%"+this.searchTerm+"%')";
                ...
                if (dataFor.equals("listdatapenjamin") || this.dataFor.equals("listdatapenjaminhome")){
                    ...
                    whereClause += " AND (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' )";
                }else{
                    whereClause += " (penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_CIF]+" LIKE '%"+this.searchTerm+"%' "
                    + "OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN]+" LIKE '%"+this.searchTerm+"%' "
                    + "OR penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_ALAMAT_PENJAMIN]+" LIKE '%"+this.searchTerm+"%')";
	...
	if(datafor.equals("listdatasearch")){
	    listData = PstPenjamin.getlistJoin(start, amount, whereClause, order, periodeId, kodeCabang);
	}else if (datafor.equals("listdatapenjamin") || this.dataFor.equals("listdatapenjaminhome")){
            listData = PstPenjamin.getlistJoin(start, amount, whereClause, order,periodeId, kodeCabang);
        }else if(datafor.equals("showlistpenjamin")){            
	    listData = PstPenjamin.getlistJoin(start, amount, whereClause, order, periodeId,kodeCabang);
	}else if(datafor.equals("searchidentitas")){
	    listData = PstPenjamin.getlistJoin(start, amount, whereClause, order, periodeId,"");
	}else if(datafor.equals("showform2")){
            listData = PstPenjamin.getlistJoin(start, amount, whereClause, order,periodeId, kodeCabang);
        }
		...
		ja.put(""+(this.start+i+1));
                ...
                if(privView){
                    buttonUpdate += "<a href='"+approot+"/dslik/perlengkapandata/perlengkapandata.jsp?command=8&oid="+penjamin.getOID()+"&type="+penjamin.getDebiturType()+"&cif="+penjamin.getCif()+"&activetab=AjaxPenjamin&otheroid="+penjamin.getOID()+"' class='btn btn-warning'><i class='fa fa-pencil'></i> View</a>";
                }
		ja.put(buttonUpdate);
		...
		ja.put(""+(this.start+i+1));
                ja.put(""+penjamin.getNoIdPenjamin());
                ja.put(""+(penjamin.getNamaIdentitas()== null ? "-" : penjamin.getNamaLengkap()));
                ja.put(""+(penjamin.getAlamatPenjamin()== null ? "-" : penjamin.getAlamatPenjamin()));
//                if(privView){
//                    buttonUpdate += "<a href='"+approot+"/dslik/perlengkapandata/perlengkapandata.jsp?command=8&oid="+penjamin.getDebiturOid()+"&type="+penjamin.getDebiturType()+"&cif="+penjamin.getCif()+"&activetab=AjaxPenjamin&otheroid="+penjamin.getOID()+"' class='btn btn-warning'><i class='fa fa-pencil'></i> View</a>";
//                }
//		ja.put(buttonUpdate);
		ja.put("<button type='button' class='btn btn-sm btn-success btn-selectnoidentitas' data-oid='"+penjamin.getOID()+"' data-for='selectidentitas' data-command='"+Command.NONE+"' data-target='AjaxPenjamin'><i class='fa fa-check'></i> Select</button>");
		array.put(ja);
	    }else if (datafor.equals("showform2")){
                penjamin = (Penjamin) listData.get(i);
		ja.put(""+(this.start+i+1));
		...
		+ "<div class='form-horizontal' style='padding:10px;background:#fff;border:5px solid #f1f1f1;'>"
			...
			+ "<div class='col-sm-8'>"
                            + "<div class='input-group'>"
                                ...
                                + "<span class='btn btn-primary input-group-addon' id='btn-showmodalmasterpenjamin' data-for='listselect' data-command='"+Command.NONE+"' data-target='AjaxMasterPenjamin'><i class='fa fa-search'></i></span>"
                                //+ "<span class='btn btn-primary input-group-addon' id='btn-showmodalnoidentitas' data-for='searchidentitas' data-command='"+Command.NONE+"' data-target='AjaxPenjamin'><i class='fa fa-search'></i></span>"
                            + "</div>"
			+ "</div>"
//                        + "<div class='col-sm-2'>"
//                            
			...
			+ "<div class='col-sm-8'>";
                            ...
                            if(listKredit.size() > 0){
                                for(int i = 0; i < listKredit.size(); i++){
                                    ...
                                    kreditVal.add("F01 - "+kredit.getNoRekening());
                                }
                            }
                            
                            Vector listBankGaransi = PstBankGaransi.list(0, 0, ""+PstBankGaransi.fieldNames[PstBankGaransi.FLD_CIF]+"='"+penjamin.getCif()+"' "
                                    + " AND "+PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID]+"='"+oidPeriodeWrite+"'", "");
                            
                            if(listBankGaransi.size() > 0){
                                for(int i = 0; i < listBankGaransi.size(); i++){
                                    BankGaransi bankGaransi = (BankGaransi) listBankGaransi.get(i);
                                    kreditKey.add(""+bankGaransi.getNoRekening());
                                    kreditVal.add("F05 - "+bankGaransi.getNoRekening());
                                }
                            }
...
//		    + "</div>"
			...
			+ "<label class='control-label col-sm-4'>"
                            ...
                            + "Jenis Identitas  <i style='color:red'>m*</i></label>"
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_JENIS_IDENTITAS] +"' id='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_JENIS_IDENTITAS] +"' value='"+penjamin.getJenisIdentitas()+"' data-required='required' readonly='' data-number='true' data-alpha='true' data-special=\" &',.()-\" data-type='text' data-error-message='"+FrmPenjamin.fieldError[FrmPenjamin.FRM_FIELD_JENIS_IDENTITAS]+"'>"
			+ "</div>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
			...
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_NAMA_IDENTITAS] +"' id='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_NAMA_IDENTITAS] +"' value='"+penjamin.getNamaIdentitas()+"' data-required='required' readonly='' data-number='true' data-alpha='true' data-special=\" &',.()-\" data-type='text' data-error-message='"+FrmPenjamin.fieldError[FrmPenjamin.FRM_FIELD_NAMA_IDENTITAS]+"'>"
			+ "</div>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
...
//		    + "</div>"
			...
			+ "<label class='control-label col-sm-4'>"
                            ...
                            + "Kode Golongan Penjamin  <i style='color:red'>m*</i></label>"
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_KODE_GOL_PENJAMIN] +"' id='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_KODE_GOL_PENJAMIN] +"' value='"+penjamin.getKodeGolPenjamin()+"' data-required='required' readonly='' data-number='true' data-alpha='true' data-special=\" ./&,()'-\" data-type='text' data-error-message='"+FrmPenjamin.fieldError[FrmPenjamin.FRM_FIELD_KODE_GOL_PENJAMIN]+"'>"
			+ "</div>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
			...
			+ "<div class='col-sm-8'>"
			    + "<input type='text' class='form-control' name='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_ALAMAT_PENJAMIN] +"' id='"+ FrmPenjamin.fieldNames[FrmPenjamin.FRM_FIELD_ALAMAT_PENJAMIN] +"' value='"+penjamin.getAlamatPenjamin()+"' data-required='required' readonly='' data-number='true' data-alpha='true' data-special=\" ./&,()'-\" data-type='text' data-error-message='"+FrmPenjamin.fieldError[FrmPenjamin.FRM_FIELD_ALAMAT_PENJAMIN]+"'>"
			+ "</div>"
		    + "</div>"
		    + "<div class='form-group'>"
			+ "<label class='control-label col-sm-4'>"
    ...
    public void getDataPenjamin(HttpServletRequest request){
        Penjamin penjamin = new Penjamin();
        try {
            penjamin = PstPenjamin.fetchExc(oid);
        } catch (Exception e) {
        ...
        try {
            ...
            jsonObject.put("no_rekening", "" + penjamin.getNoRekening());
            jsonObject.put("cif", "" + penjamin.getCif());
            jsonObject.put("kode_jenis_segmen", "" + penjamin.getKodeJenisSegmentFasilitas());
            ...
            this.jSONArrayPenjamin.put(jsonObject);
...
//        }
        ...
        int summary = PstPenjamin.getCountJoin(whereClause,periodeId,kodeCabang);
        returnData +=""+summary;
        return returnData;
    }
