...
public class PstAgunan extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_AGUNAN_INITIAL = "dslik_agunan_initial";
    ...
    public static Vector listJoinParentCabangIntial(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
                    + "TRIM(AGUNAN_OID) AS AGUNAN_OID," +
                        ...
                        "TRIM(PROSENTASE_PARIPASU) AS PROSENTASE_PARIPASU, STATUS_OPERASI_DATA, KODE_JENIS_SEGMENT_FASILITAS,"
                      + "TRIM(PARENT_CODE) AS PARENT_CODE "  
                      + "FROM " + TBL_AGUNAN_INITIAL+" "
                      + "INNER JOIN dslik_cabang_bank ON "+TBL_AGUNAN_INITIAL+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
                    //+ " FROM " + TBL_AGUNAN;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
    ...
    public static Vector listJoinParentCabangAgunanHapusNew(long periodeIdCurrent, long periodeIdPass) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql ="SELECT DISTINCT "+ 
                        ...
                        "FROM dslik_agunan "+ 
                        "INNER JOIN dslik_cabang_bank ON "+TBL_AGUNAN+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG "+
                        "WHERE dslik_agunan.PERIODE_ID IN ('"+periodeIdPass+"','"+periodeIdCurrent+"') AND (dslik_agunan.NO_REKENING,dslik_agunan.CIF) IN " +
                        "( " +
                        "SELECT dslik_kredit.NO_REKENING, dslik_kredit.CIF FROM dslik_kredit WHERE dslik_kredit.KODE_KONDISI='02' AND PERIODE_ID IN ('"+periodeIdCurrent+"')  " +
                        "AND (NO_REKENING, CIF) IN (SELECT NO_REKENING, CIF FROM dslik_agunan WHERE periode_id IN ('"+periodeIdPass+"','"+periodeIdCurrent+"')) " +
                        ")";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
    ...
    public static Vector listJoinParentCabangAgunanHapusBelumLunas(long periodeIdCurrent, long periodeIdPass) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql ="SELECT DISTINCT "+ 
                        "TRIM(dslik_agunan.AGUNAN_OID) AS AGUNAN_OID," +
                        "TRIM(dslik_agunan.FLAG_DETAIL) AS FLAG_DETAIL," +
                        "TRIM(dslik_agunan.KODE_REGISTER_AGUNAN) AS KODE_REGISTER_AGUNAN," +
                        "TRIM(dslik_agunan.NO_REKENING) AS NO_REKENING," +
                        "TRIM(dslik_agunan.CIF) AS CIF," +
                        "TRIM(dslik_agunan.KODE_STATUS_AGUNAN) AS KODE_STATUS_AGUNAN," +
                        "TRIM(dslik_agunan.KODE_JENIS_AGUNAN) AS KODE_JENIS_AGUNAN," +
                        "TRIM(dslik_agunan.PERINGKAT_AGUNAN) AS PERINGKAT_AGUNAN," +
                        "TRIM(dslik_agunan.KODE_LEMBAGA_PEMERINGKAT) AS KODE_LEMBAGA_PEMERINGKAT," +
                        "TRIM(dslik_agunan.KODE_JENIS_PENGIKATAN) AS KODE_JENIS_PENGIKATAN," +
                        "TRIM(dslik_agunan.TGL_PENGIKATAN) AS TGL_PENGIKATAN," +
                        "TRIM(dslik_agunan.NAMA_PEMILIK_AGUNAN) AS NAMA_PEMILIK_AGUNAN," +
                        "TRIM(dslik_agunan.BUKTI_KEPEMILIKAN) AS BUKTI_KEPEMILIKAN," +
                        "TRIM(dslik_agunan.ALAMAT_AGUNAN) AS ALAMAT_AGUNAN," +
                        "TRIM(dslik_agunan.KODE_KAB_LOKASI_AGUNAN) AS KODE_KAB_LOKASI_AGUNAN," +
                        "TRIM(dslik_agunan.NILAI_AGUNAN_NJOP) AS NILAI_AGUNAN_NJOP," +
                        "TRIM(dslik_agunan.NILAI_AGUNAN_LJK) AS NILAI_AGUNAN_LJK," +
                        "TRIM(dslik_agunan.TGL_PENILAIAN_LJK) AS TGL_PENILAIAN_LJK," +
                        "TRIM(dslik_agunan.NILAI_AGU_PENILAI_INDEP) AS NILAI_AGU_PENILAI_INDEP," +
                        "TRIM(dslik_agunan.NAMA_PENILAI_INDEP) AS NAMA_PENILAI_INDEP," +
                        "TRIM(dslik_agunan.TGL_PENILAIAN_PENILAI_INDEP) AS TGL_PENILAIAN_PENILAI_INDEP," +
                        "TRIM(dslik_agunan.STATUS_PARIPASU) AS STATUS_PARIPASU," +
                        "TRIM(dslik_agunan.STATUS_KREDIT_JOIN) AS STATUS_KREDIT_JOIN," +
                        "TRIM(dslik_agunan.DIASURANSIKAN) AS DIASURANSIKAN," +
                        "TRIM(dslik_agunan.KETERANGAN) AS KETERANGAN," +
                        "TRIM(dslik_agunan.KODE_KANTOR_CABANG) AS KODE_KANTOR_CABANG," +
                        "TRIM(dslik_agunan.OPERASI_DATA) AS OPERASI_DATA," +
                        "TRIM(dslik_agunan.OPEN_DATE) AS OPEN_DATE," +
                        "TRIM(dslik_agunan.STATUS_DATA) AS STATUS_DATA," +
                        "TRIM(dslik_agunan.PERIODE_ID) AS PERIODE_ID," +
                        "TRIM(PROSENTASE_PARIPASU) AS PROSENTASE_PARIPASU, "+ 
                        "dslik_agunan.STATUS_OPERASI_DATA AS STATUS_OPERASI_DATA, "+ 
                        "dslik_agunan.KODE_JENIS_SEGMENT_FASILITAS AS KODE_JENIS_SEGMENT_FASILITAS,"+ 
                        "TRIM(PARENT_CODE) AS PARENT_CODE "+ 
                        "FROM  (" +
                        "	SELECT * FROM dslik_agunan WHERE periode_id='"+periodeIdPass+"' AND (NO_REKENING, CIF, KODE_REGISTER_AGUNAN) NOT IN " +
                        "		(" +
                        "			SELECT NO_REKENING, CIF, KODE_REGISTER_AGUNAN FROM dslik_agunan WHERE PERIODE_ID='"+periodeIdCurrent+"'" +
                        "		)" +
                        "	) " +
                        "AS dslik_agunan " +
                        "INNER JOIN dslik_kredit ON dslik_agunan.NO_REKENING=dslik_kredit.NO_REKENING " +
                        "INNER JOIN dslik_cabang_bank ON dslik_agunan.KODE_KANTOR_CABANG=dslik_cabang_bank.KODE_CABANG " +
                        "WHERE dslik_kredit.PERIODE_ID='"+periodeIdCurrent+"' " +
                        "AND dslik_kredit.KODE_KONDISI='00' "+ 
                        "AND dslik_agunan.CIF IN (SELECT CIF FROM dslik_debitur WHERE PERIODE_ID='"+periodeIdCurrent+"')";
    ...
    public static int checkCountParipasu(long periodeId, String kodeRegisterAgunan) {
        DBResultSet dbrs = null;
        int result = 0;
        try {
            String sql = "SELECT COUNT(AGUNAN_OID) AS TOT FROM " + TBL_AGUNAN + " WHERE "
    ...
    public static long updateStatusDataNotComplate(long oidsegment) throws DBException {
        if(oidsegment==0){
            return 0;
        }
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_AGUNAN+" SET STATUS_DATA=NULL WHERE "+fieldNames[FLD_AGUNAN_OID]+"='"+oidsegment+"'";
        try {
            iResult = DBHandler.execUpdate(stSql);
        } catch (DBException e) {
            e.printStackTrace();
    ...
    public static boolean checkDataAgunanPrevPeriode(long PeriodeId, String kodeRegisterAgunan, String noRekening, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_AGUNAN + " WHERE "
                    + PstAgunan.fieldNames[PstAgunan.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstAgunan.fieldNames[PstAgunan.FLD_KODE_REGISTER_AGUNAN] + " = '" + kodeRegisterAgunan+"'"
                    + " AND "+PstAgunan.fieldNames[PstAgunan.FLD_NO_REKENING] + " = '" + noRekening+"'"
                    + " AND "+PstAgunan.fieldNames[PstAgunan.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
