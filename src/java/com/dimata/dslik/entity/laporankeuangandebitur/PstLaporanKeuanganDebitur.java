...
public class PstLaporanKeuanganDebitur extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public void setSqlQueryHistory(String sqlQueryHistory) {
     ...
     public static boolean checkDataLaporanKeuanganPrevPeriode(long PeriodeId, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_LAPORAN_KEUANGAN_DEBITUR+ " WHERE "
                    + PstLaporanKeuanganDebitur.fieldNames[PstLaporanKeuanganDebitur.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstLaporanKeuanganDebitur.fieldNames[PstLaporanKeuanganDebitur.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
    ...
    public static int getCountJoin(String whereClause, long periodeId, String kodecabang) {
        ...
        try {
            String sql = "SELECT DISTINCT COUNT(laporan." + PstLaporanKeuanganDebitur.fieldNames[PstLaporanKeuanganDebitur.FLD_LAPORAN_KEUANGAN_DEBITUR_OID] + ") "
                ...
                if(!kodecabang.equals("")){
                    ...
                    ") AS dup " +
                    "ON laporan.CIF=dup.CIF";
                    
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND laporan.PERIODE_ID='"+periodeId+"'";
                sql = sql + " AND laporan.CIF IN (SELECT CIF FROM dslik_laporan_keuangan_debitur WHERE periode_id='"+periodeId+"') ";
            }
     ...
     public static Vector getlistJoin(int limitStart, int recordToGet,String whereClause, String order,  long periodeId, String kodecabang) {
        ...
        try {
            String sql = "SELECT DISTINCT "
                ...
                if(!kodecabang.equals("")){
                    ...
                    sql = sql +" GROUP BY CIF ) AS CUP " +
                            " ORDER BY CUP.TGL_AWAL DESC "+
                    ") AS dup " +
                    "ON laporan.CIF=dup.CIF inner join dslik_debitur on dslik_debitur.CIF=laporan.CIF and dslik_debitur.PERIODE_ID='"+periodeId+"'";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND laporan.PERIODE_ID='"+periodeId+"'";
                sql = sql + " AND laporan.CIF IN (SELECT CIF FROM dslik_laporan_keuangan_debitur WHERE PERIODE_ID='"+periodeId+"') ";
            ...
            while (rs.next()) {
               // count = rs.getInt(1);
                LaporanKeuanganDebitur entPenjamin = new LaporanKeuanganDebitur();
