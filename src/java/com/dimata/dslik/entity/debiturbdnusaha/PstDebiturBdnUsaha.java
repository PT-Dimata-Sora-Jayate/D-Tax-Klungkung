...
public class PstDebiturBdnUsaha extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_DEBITUR_BDN_USAHA_INITIAL = "dslik_debitur_initial";
...
//                    + " FROM " + TBL_DEBITUR_BDN_USAHA;
            ...
            while (rs.next()) {
                ...
                if(isUpdate || isActive){
                    boolean checkKodeKondisiKredit = PstDebiturIndividu.checkKodeKondisiFasilitasKredit(PeriodeId, entDebiturBdnUsaha.getCif());
                    boolean checkKodeKondisiBankGaransi = PstDebiturIndividu.checkKodeKondisiFasilitasBankGaransi(PeriodeId, entDebiturBdnUsaha.getCif());
...
//                    + " FROM " + TBL_DEBITUR_BDN_USAHA;
                      ...
                      + "STATUS_OPERASI_DATA, "
                      + "TRIM(PARENT_CODE) AS PARENT_CODE "  
                      + "FROM " + TBL_DEBITUR_BDN_USAHA_INITIAL+" "
                      + "INNER JOIN dslik_cabang_bank ON "+TBL_DEBITUR_BDN_USAHA_INITIAL+"."+fieldNames[FLD_KODE_KTR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
...
//                        " ) AS dup" +
    ...
    public static long updateStatusDataNotComplate(long oidDebitur) throws DBException {
        if(oidDebitur==0){
            return 0;
        }
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_DEBITUR_BDN_USAHA+" SET STATUS_DATA=NULL WHERE "+PstDebiturBdnUsaha.fieldNames[PstDebiturBdnUsaha.FLD_DEBITUR_BDN_USHA_OID]+"='"+oidDebitur+"'";
        try {
            iResult = DBHandler.execUpdate(stSql);
        } catch (DBException e) {
            e.printStackTrace();
    ...
    public static long updateStatusOperasiData(long oidDebitur, int statusOperasiData) throws DBException {
      ...
      public static boolean checkDataDebiturBdnUsahaPrevPeriode(long PeriodeId, String cif, String periodeIn) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_DEBITUR_BDN_USAHA + " WHERE "
                    + PstDebiturBdnUsaha.fieldNames[PstDebiturBdnUsaha.FLD_PERIODE_ID] + " IN (" + periodeIn+")"
                    + " AND "+PstDebiturBdnUsaha.fieldNames[PstDebiturBdnUsaha.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
    ...
    public static Vector listJoinHapusData(int limitStart, int recordToGet, String whereClause, String order, long periodeId, String kodecabang) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
