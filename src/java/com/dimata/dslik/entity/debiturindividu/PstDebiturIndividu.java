...
public class PstDebiturIndividu extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_DEBITUR_INDIVIDU_INITIAL = "dslik_debitur_initial";
    ...
    public static Vector listJoinParentCabang(int limitStart, int recordToGet, String whereClause, String order, boolean isUpdate, long PeriodeId, boolean activeUpdate) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT TRIM(FLAG_DETAIL)," +
            ...
            while (rs.next()) {
                ...
                //cek apakah mempunya nasabah yg aktive
                if(isUpdate || activeUpdate){
                    boolean checkKodeKondisiKredit = checkKodeKondisiFasilitasKredit(PeriodeId, entDebiturIndividu.getCif());
                    boolean checkKodeKondisiBankGaransi = checkKodeKondisiFasilitasBankGaransi(PeriodeId, entDebiturIndividu.getCif());
     ...
     public static boolean checkKodeKondisiFasilitasKredit(long PeriodeId, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM dslik_kredit  WHERE "
                    + PstKredit.fieldNames[PstKredit.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstKredit.fieldNames[PstKredit.FLD_CIF]+"='"+cif+"'"+
                    " AND KODE_KONDISI NOT IN ('01','02','05','06','07','08','09','11','12','14')";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true; //ada yang aktive
            }
            rs.close();
        } catch (Exception e) {
     ...
     public static boolean checkKodeKondisiFasilitasBankGaransi(long PeriodeId, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM dslik_bank_garansi WHERE "
                    + PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstBankGaransi.fieldNames[PstBankGaransi.FLD_CIF]+"='"+cif+"'"+
                      " AND KODE_KONDISI NOT IN ('01','02','05','06','07','08','09','11','12','14')";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
    ...
    public static Vector listJoinParentCabangIntial(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            String sql = "SELECT DISTINCT TRIM(FLAG_DETAIL)," +
                      ...
                      + "TRIM(STATUS_DATA) AS STATUS_DATA, "
                      + "TRIM(PARENT_CODE) AS PARENT_CODE "  
                      + "FROM " + TBL_DEBITUR_INDIVIDU_INITIAL+" "
                      + " INNER JOIN dslik_cabang_bank ON "+TBL_DEBITUR_INDIVIDU_INITIAL+".KODE_KANTOR_CABANG=dslik_cabang_bank.KODE_CABANG ";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
...
//                        ") AS dup " +
    ...
    public static boolean checkDataDebiturPrevPeriode(long PeriodeId, String cif, String periodeIn) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_DEBITUR_INDIVIDU + " WHERE "
                    + PstDebiturIndividu.fieldNames[PstDebiturIndividu.FLD_PERIODE_ID] + " IN (" + periodeIn+")"
                    + " AND "+PstDebiturIndividu.fieldNames[PstDebiturIndividu.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
    ...
    public static Vector listJoin(int limitStart, int recordToGet, String whereClause, String order, long periodeId, String kodecabang) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT TRIM(dslik_debitur.FLAG_DETAIL)," +
    ...
    public static long updateKodePekerjaan(long oidsegment, String kodePekerjaan) throws DBException {
        if(oidsegment==0){
            return 0;
        }
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_DEBITUR_INDIVIDU+" SET KODE_PEKERJAAN='"+kodePekerjaan+"' WHERE "+fieldNames[FLD_DEBITUR_INDIVIDU_OID]+"='"+oidsegment+"'";
        try {
