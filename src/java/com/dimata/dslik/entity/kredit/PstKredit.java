...
public class PstKredit extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_KREDIT_LUNAS = "dslik_kredit_initial";
    ...
    public static Kredit fetchExc(long oid) throws DBException {
        try {
            ...
            entKredit.setProsentaseBunga(pstKredit.getdouble(FLD_PROSENTASE_BUNGA));
    ...
    public static synchronized long updateExc(Kredit entKredit) throws DBException {
        try {
            if (entKredit.getOID() != 0) {
                ...
                pstKredit.setDouble(FLD_PROSENTASE_BUNGA, entKredit.getProsentaseBunga());
    ...
    public static void resultToObject(ResultSet rs, Kredit entKredit) {
        try {
            ...
            entKredit.setProsentaseBunga(rs.getDouble(PstKredit.fieldNames[PstKredit.FLD_PROSENTASE_BUNGA]));
...
//        }
    ...
    public static Vector listParentCabangIntial(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+ 
                        ...
                        + "TRIM(STATUS_DATA) AS STATUS_DATA, "
                        + "TRIM(PARENT_CODE) AS PARENT_CODE, STATUS_OPERASI_DATA "  
                        + "FROM " + TBL_KREDIT_LUNAS+" "
                        + "INNER JOIN dslik_cabang_bank ON "+TBL_KREDIT_LUNAS+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
//                        + "STATUS_OPERASI_DATA"
//                        + " FROM " + TBL_KREDIT;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
    ...
    public static int getCountLunas(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT COUNT(" + PstKredit.fieldNames[PstKredit.FLD_KREDIT_OID] + ") FROM " + TBL_KREDIT_LUNAS;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
    ...
    public static long updateProsentaseBunga(long oidsegment, double prosentaseBunga) throws DBException {
        if(oidsegment==0){
            return 0;
        }
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_KREDIT+" SET PROSENTASE_BUNGA='"+prosentaseBunga+"' WHERE "+fieldNames[FLD_KREDIT_OID]+"='"+oidsegment+"'";
        try {
            iResult = DBHandler.execUpdate(stSql);
        } catch (DBException e) {
            e.printStackTrace();
    ...
    public static long updateStatusDataNotComplate(long oidsegment) throws DBException {
        ...
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_KREDIT+" SET STATUS_DATA=NULL WHERE "+fieldNames[FLD_KREDIT_OID]+"='"+oidsegment+"'";
        try {
            iResult = DBHandler.execUpdate(stSql);
        } catch (DBException e) {
            e.printStackTrace();
    ...
    public static long moveDataAgunanHapus(long periodeId, String noRekening) throws DBException {
     ...
     public static boolean checkDataDebiturBdnUsahaPrevPeriode(long PeriodeId, String noRekening, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_KREDIT + " WHERE "
                    + PstKredit.fieldNames[PstKredit.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstKredit.fieldNames[PstKredit.FLD_NO_REKENING]+"='"+noRekening+"'"
