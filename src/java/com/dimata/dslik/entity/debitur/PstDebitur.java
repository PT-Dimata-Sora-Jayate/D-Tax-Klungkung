...
//                    + "AND periode."+PstPeriode.fieldNames[PstPeriode.FLD_POSTED]+"='0'";
    ...
    public static boolean checkCifExisting(long periodeId, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT DISTINCT * FROM " + TBL_DEBITUR_INDIVIDU + " WHERE "
                    + PstDebitur.fieldNames[PstDebitur.FLD_PERIODE_ID] + " = '" + periodeId+"'"
    ...
    public static long checkDebiturOidCifExisting(long periodeId, String cif) {
        DBResultSet dbrs = null;
        long result = 0;
        try {
            String sql = "SELECT DISTINCT * FROM " + TBL_DEBITUR_INDIVIDU + " WHERE "
            ...
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getLong(PstDebitur.fieldNames[PstDebitur.FLD_DEBITUR_OID]);
            }
            rs.close();
        } catch (Exception e) {
            System.out.println("err : " + e.toString());
    ...
    public static boolean checkCif(String where) {
        DBResultSet dbrs = null;
        boolean result = false;
