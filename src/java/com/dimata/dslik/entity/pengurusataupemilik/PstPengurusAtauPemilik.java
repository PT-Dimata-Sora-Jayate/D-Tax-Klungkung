...
public class PstPengurusAtauPemilik extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_PENGURUS_ATAU_PEMILIK_INITIAL = "dslik_pengurus_atau_pemilik_initial";
    ...
    public static Vector listJoinParentCabang(int limitStart, int recordToGet, String whereClause, String order, boolean isUpdate, long periodeId, boolean isActive) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
            ...
            while (rs.next()) {
                ...
                if(isUpdate || isActive){
                    boolean checkKodeKondisiKredit = PstDebiturIndividu.checkKodeKondisiFasilitasKredit(periodeId, entPengurusAtauPemilik.getCif());
                    boolean checkKodeKondisiBankGaransi = PstDebiturIndividu.checkKodeKondisiFasilitasBankGaransi(periodeId, entPengurusAtauPemilik.getCif());
        ...
        try {
            String sql = "SELECT DISTINCT "
                    + "TRIM(PENGURUS_OID) AS PENGURUS_OID," +
                        ...
                        "TRIM( PERIODE_ID) AS  PERIODE_ID, STATUS_OPERASI_DATA,"
                      + "TRIM(PARENT_CODE) AS PARENT_CODE "  
                      + "FROM " + TBL_PENGURUS_ATAU_PEMILIK_INITIAL+" "
                      + "INNER JOIN dslik_cabang_bank ON "+TBL_PENGURUS_ATAU_PEMILIK_INITIAL+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
                    //+ " FROM " + TBL_PENGURUS_ATAU_PEMILIK;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
    ...
    public static int getSumPangsa(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT SUM(" + PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PANGSA_KEPEMILIKAN] + ") FROM " + TBL_PENGURUS_ATAU_PEMILIK;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
    ...
    public static int getCountPengurusPemilik(String whereClause, long periodeId, String kodecabang) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT COUNT(pengurus." + PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PENGURUS_ATAU_PEMILIK_OID] + ") "
                    + "FROM " + TBL_PENGURUS_ATAU_PEMILIK+" AS pengurus "
                    + " INNER JOIN " +
                        "(" +
                ...
                if(!kodecabang.equals("")){
                    ...
                    ") AS dup " +
                    "ON pengurus.CIF=dup.CIF";
                    
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND pengurus.PERIODE_ID='"+periodeId+"'";
                sql = sql + " AND pengurus.CIF IN (SELECT CIF FROM dslik_pengurus_atau_pemilik WHERE periode_id='"+periodeId+"') ";
            }
    ...
    public static int getCountPangsaKurangLebihSeratus(String whereClause, long periodeId, String kodecabang) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT pengurus.* "
                    + "FROM " + TBL_PENGURUS_ATAU_PEMILIK+" AS pengurus "
                    + " INNER JOIN " +
                        "(" +
                            "SELECT DISTINCT CIF FROM (" +
            ...
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND pengurus.KODE_JABATAN in ('01','02','03','04','05','06','07','08','09','10','11','12','13','19') ";
                sql = sql + " AND pengurus.STATUS_PENGURUS=1";
                sql = sql + " AND pengurus.CIF IN (SELECT CIF FROM dslik_pengurus_atau_pemilik WHERE periode_id='"+periodeId+"') ";
            }
            
            sql=sql+" GROUP BY pengurus.CIF HAVING (SUM(pengurus.PANGSA_KEPEMILIKAN) > 100 OR SUM(pengurus.PANGSA_KEPEMILIKAN) < 100)";
    ...
    public static Vector getlistJoin(int limitStart, int recordToGet, String order, String whereClause, long periodeId, String kodecabang) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
                ...
                if(!kodecabang.equals("")){
                    ...
                    sql = sql +" GROUP BY CIF ) AS CUP " +
                            " ORDER BY CUP.TGL_AWAL DESC "+
                    ") AS dup " +
                    "ON pengurus.CIF=dup.CIF inner join dslik_debitur on dslik_debitur.CIF=pengurus.CIF and dslik_debitur.PERIODE_ID='"+periodeId+"'";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND pengurus.PERIODE_ID='"+periodeId+"'";
                sql = sql + " AND pengurus.CIF IN (SELECT CIF FROM dslik_pengurus_atau_pemilik WHERE PERIODE_ID='"+periodeId+"') ";
    ...
    public static Vector getlistJoinPangsa(int limitStart, int recordToGet, String order, String whereClause, long periodeId, String kodecabang) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
    ...
    public static boolean checkCIF(String cif) {
     ...
     public static boolean checkDataPengurusPemilikPrevPeriode(long PeriodeId,String noIdentitas, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_PENGURUS_ATAU_PEMILIK + " WHERE "
                    + PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_NO_IDENTITAS] + " = '" + noIdentitas+"'"
                    + " AND "+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
