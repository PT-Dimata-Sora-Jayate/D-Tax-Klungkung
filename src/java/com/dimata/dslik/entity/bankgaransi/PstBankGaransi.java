...
public class PstBankGaransi extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_BANK_GARANSI_LUNAS = "dslik_bank_garansi_initial";
    ...
    public static Vector listJoinParentCabangInitial(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            String sql = "SELECT DISTINCT "
                    ...
                    //+ " FROM " + TBL_BANK_GARANSI;
                      + "TRIM(PARENT_CODE) AS PARENT_CODE "  
                      + "FROM " + TBL_BANK_GARANSI_LUNAS+" "
                      + "INNER JOIN dslik_cabang_bank ON "+TBL_BANK_GARANSI_LUNAS+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
    ...
    public static int getCountLunas(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT COUNT(" + PstBankGaransi.fieldNames[PstBankGaransi.FLD_BANK_GARANSI_OID] + ") FROM " + TBL_BANK_GARANSI_LUNAS;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
    ...
    public static long updateStatusDataNotComplate(long oidDebitur) throws DBException {
        ...
        int iResult = 0;
        DBResultSet dbrs = null;
        String stSql = "UPDATE "+TBL_BANK_GARANSI+" SET STATUS_DATA=NULL WHERE "+PstBankGaransi.fieldNames[PstBankGaransi.FLD_BANK_GARANSI_OID]+"='"+oidDebitur+"'";
        try {
            iResult = DBHandler.execUpdate(stSql);
        } catch (DBException e) {
            e.printStackTrace();
    ...
    public static boolean checkDataBankGaransiPrevPeriode(long PeriodeId, String noRekening, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_BANK_GARANSI + " WHERE "
                    + PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstBankGaransi.fieldNames[PstBankGaransi.FLD_NO_REKENING]+"='"+noRekening+"'"
