...
public class PstPenjamin extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_PENJAMIN_INITIAL = "dslik_penjamin_inital";
    ...
    public static Vector listJoinParentCabang(int limitStart, int recordToGet, String whereClause, String order, long periodeId, boolean isUpdate, boolean isActive) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT dslik_penjamin.PENJAMIN_OID," +
                         "dslik_penjamin.FLAG_DETAIL," +
                         "dslik_penjamin.NO_ID_PENJAMIN," +
                         "dslik_penjamin.NO_REKENING," +
                         "dslik_penjamin.CIF," +
                         "dslik_penjamin.JENIS_IDENTITAS," +
                         "dslik_penjamin.NAMA_IDENTITAS," +
                         "dslik_penjamin.NAMA_LENGKAP," +
                         "dslik_penjamin.KODE_GOL_PENJAMIN," +
                         "dslik_penjamin.ALAMAT_PENJAMIN," +
                         "dslik_penjamin.PROSENTASE_DIJAMIN," +
                         "dslik_penjamin.KETERANGAN," +
                         "dslik_cabang_bank.PARENT_CODE AS KODE_KANTOR_CABANG," +
                         "dslik_penjamin.OPERASI_DATA," +
                         "dslik_penjamin.OPEN_DATE," +
                         "dslik_penjamin.STATUS_OPERASI_DATA," +
                         "dslik_penjamin.PERIODE_ID," +
                         "dslik_penjamin.STATUS_DATA," +
                         "dslik_penjamin.KODE_JENIS_SEGMENT_FASILITAS FROM " + TBL_PENJAMIN
                         + " INNER JOIN dslik_cabang_bank ON "+TBL_PENJAMIN+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG"
                         + " INNER JOIN dslik_kredit ON dslik_kredit.NO_REKENING=dslik_penjamin.NO_REKENING AND dslik_kredit.PERIODE_ID='"+periodeId+"' ";
            ...
            while (rs.next()) {
                ...
                if(isUpdate || isActive){
                    boolean checkKodeKondisiKredit = PstDebiturIndividu.checkKodeKondisiFasilitasKredit(periodeId, entPenjamin.getCif());
                    boolean checkKodeKondisiBankGaransi = PstDebiturIndividu.checkKodeKondisiFasilitasBankGaransi(periodeId, entPenjamin.getCif());
                    if(checkKodeKondisiKredit || checkKodeKondisiBankGaransi){
                        lists.add(entPenjamin);
                    }
                }else{
                   lists.add(entPenjamin);
    ...
    public static Vector listJoinParentCabangInitial(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT PENJAMIN_OID," +
                         "FLAG_DETAIL," +
                         "NO_ID_PENJAMIN," +
                         "NO_REKENING," +
                         "CIF," +
                         ...
                         "PERIODE_ID," +
                         "STATUS_DATA," +
                         "KODE_JENIS_SEGMENT_FASILITAS FROM " + TBL_PENJAMIN_INITIAL
                         + " INNER JOIN dslik_cabang_bank ON "+TBL_PENJAMIN_INITIAL+"."+fieldNames[FLD_KODE_KANTOR_CABANG]+"=dslik_cabang_bank.KODE_CABANG";
    ...
    public static Vector getlistJoin(int limitStart, int recordToGet,String whereClause, String order,  long periodeId, String kodecabang) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "
                    + " dslik_debitur."+PstDebitur.fieldNames[PstDebitur.FLD_DEBITUR_OID] +","
                    + " dslik_debitur."+PstDebitur.fieldNames[PstDebitur.FLD_KODE_JENIS_NSB]+","
                    + " penjamin.* "
                ...
                if(!kodecabang.equals("")){
                    ...
                    ") AS dup " +
                    "ON penjamin.CIF=dup.CIF inner join dslik_debitur on dslik_debitur.CIF=penjamin.CIF and dslik_debitur.PERIODE_ID='"+periodeId+"'";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
                sql = sql + " AND penjamin.PERIODE_ID='"+periodeId+"'";
                sql = sql + " AND penjamin.CIF IN (SELECT CIF FROM dslik_penjamin WHERE PERIODE_ID='"+periodeId+"') ";
            }
    ...
    public static Vector listJoinx(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
    ...
    public static int getCountJoin(String whereClause, long periodeId, String kodecabang) {
        DBResultSet dbrs = null;
        try {
    ...
    public static int getCountJoinx(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT COUNT(penjamin." + PstPenjamin.fieldNames[PstPenjamin.FLD_PENJAMIN_OID] + ") "
                    + "FROM " + TBL_PENJAMIN+" AS penjamin "
    ...
    public void setSqlQueryHistory(String sqlQueryHistory) {
     ...
     public static boolean checkDataPenjaminPrevPeriode(long PeriodeId,String noIdPenjamin, String noRekening, String cif) {
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT * FROM " + TBL_PENJAMIN + " WHERE "
                    + PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID] + " = '" + PeriodeId+"'"
                    + " AND "+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN] + " = '" + noIdPenjamin+"'"
                    + " AND "+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_REKENING] + " = '" + noRekening+"'"
                    + " AND "+PstPenjamin.fieldNames[PstPenjamin.FLD_CIF]+"='"+cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = true;
