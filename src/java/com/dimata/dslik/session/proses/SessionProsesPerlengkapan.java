...
public class SessionProsesPerlengkapan {
    ...
    public static int getCountJoinDebiturMenikahTidakPunyaPasangan(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql ="SELECT  DISTINCT COUNT(DISTINCT l.CIF) FROM dslik_kredit l INNER JOIN dslik_debitur r " +
                        "ON r.CIF = l.CIF " +
                        "WHERE 1=1 ";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " AND " + whereClause;
    ...
    public static Vector getListJoinDebiturMenikahTidakPunyaPasangan(int limitStart, int recordToGet, String whereClause) {
        DBResultSet dbrs = null;
        Vector lists = new Vector();
        
        try {
            String sql = " SELECT DISTINCT r.DEBITUR_OID, r.KODE_JENIS_NSB, l.CIF, r.NIK, r.NAMA_IDENTITAS "
                       + " FROM dslik_kredit l INNER JOIN dslik_debitur r " +
                         " ON r.CIF = l.CIF " +
                         " WHERE 1=1 ";
            ...
            while (rs.next()) {
                ...
                entDebitur.setOID(rs.getLong("DEBITUR_OID"));
                entDebitur.setKodeJenisNsb(rs.getInt("KODE_JENIS_NSB"));
                entDebitur.setCif(rs.getString("CIF"));
                entDebitur.setNik(rs.getString("NIK"));
                entDebitur.setNamaIdentitas(rs.getString("NAMA_IDENTITAS"));
    ...
    public static int getCountJoinLaporanKeuanganWajib(long periodeId, String cabang) {
        DBResultSet dbrs = null;
        try {
            String sql =" SELECT dslik_kredit.CIF, dslik_kredit.NO_REKENING, SUM(dslik_kredit.PLAFON) AS PLAFON_TOT  " +
                        " FROM dslik_kredit " +
                        " WHERE dslik_kredit.PERIODE_ID ='"+periodeId+"' ";
                        if(!cabang.equals("")){
                            sql = sql + " AND dslik_kredit.KODE_KANTOR_CABANG = '"+cabang+"' ";
                        }
                        sql = sql + " AND dslik_kredit.CIF NOT IN (SELECT CIF FROM dslik_laporan_keuangan_debitur WHERE PERIODE_ID='"+periodeId+"') " +
                        " GROUP BY dslik_kredit.CIF HAVING SUM(dslik_kredit.PLAFON) >=5000000000";
    ...
    public static Vector getListJoinJoinLaporanKeuanganWajib(long periodeId, String cabang) {
        DBResultSet dbrs = null;
        Vector lists = new Vector();
        ...
        try {
            ...
            while (rs.next()) {
                Debitur entDebitur = new Debitur();
                String where = ""+PstDebitur.fieldNames[PstDebitur.FLD_CIF]+"='"+rs.getString("CIF")+"' AND "+PstDebitur.fieldNames[PstDebitur.FLD_PERIODE_ID]+"='"+periodeId+"'";
                Vector list = PstDebitur.list(0, 0, where, "");
                if(list.size()>0){
                    entDebitur = (Debitur) list.get(0);
                }
                lists.add(entDebitur);
            }
            rs.close();
    ...
    public static int getCountJoinPengurusPemilik(String whereClause, long periodeId) {
        DBResultSet dbrs = null;
        try {
            String sql ="SELECT  DISTINCT COUNT(DISTINCT l."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+") FROM "+PstPengurusAtauPemilik.TBL_PENGURUS_ATAU_PEMILIK+" l "
                    + "LEFT JOIN "+PstDebitur.TBL_DEBITUR_INDIVIDU+" r " +
                        "ON r."+PstDebitur.fieldNames[PstDebitur.FLD_CIF]+" = l."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_CIF]+" AND r.PERIODE_ID='"+periodeId+"' " +
                        "WHERE r."+PstDebitur.fieldNames[PstDebitur.FLD_CIF]+" IS NULL ";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " AND " + whereClause;
            }
    ...
    public static Vector listOtherPengurusPemilik(int limitStart, int recordToGet, String whereClause, String order, long periodeId) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
