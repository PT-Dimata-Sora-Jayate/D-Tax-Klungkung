...
public class SessGetContentDataOjk {
    ...
    public static String getContentDataKodeBidangUsaha (String kodeKantorCabCoreBanking) {
        ...
        try {
            ...
            //cek jika bidang usaha adalah 6 digit
            if(result.equals("")){
                result = getContentDataKodeBidangUsahaEnamDigit(xData);
            }
        } catch (Exception e) {
            ////System.out.println("err : " + e.toString());            
        } finally {
    ...
    //Kode Bidang Usaha 
    public static String getContentDataKodeBidangUsahaEnamDigit (String kodeKantorCabCoreBanking) {
        DBResultSet dbrs = null;
        String result = "";
        
        try {
            String xData=kodeKantorCabCoreBanking.replace("\\s", "");
            String sql = " SELECT * FROM " + PstContentDataBidangUsahaEnamDigit.TBL_CONTENT_DATA_BIDANG_USAHA + " WHERE "
                    + PstContentDataBidangUsahaEnamDigit.fieldNames[PstContentDataBidangUsahaEnamDigit.FLD_KODE_CORE_BANKING] + 
                    " = '" + xData+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getString(PstContentDataBidangUsahaEnamDigit.fieldNames[PstContentDataBidangUsahaEnamDigit.FLD_KODE_OJK]);
            }
            rs.close();
        } catch (Exception e) {        
        } finally {
    ...
    public static String getContentDataKecamatan(String kodeKecamatan, String kodeDati) {
        DBResultSet dbrs = null;
        String result = ""+kodeKecamatan;
        try {
            String xData=kodeKecamatan.replace("\\s", "");
            String sql = "SELECT * FROM dslik_cd_kecamatan WHERE "
                    + PstContentDataHubDgnPelapor.fieldNames[PstContentDataHubDgnPelapor.FLD_KODE_CORE_BANKING] + " = '" + xData+"' "
                    + " AND KODE_DATI_II='"+kodeDati+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getString("NAMA_KECAMATAN");
            }
            rs.close();
        } catch (Exception e) {
            System.out.println("err : " + e.toString());
    ...
    public static int getOperasiDataDebiturKredit(String cif,long periode_id) {
        DBResultSet dbrs = null;
        int result = -1;
        try {
            String sql = "SELECT * FROM "+PstDebitur.TBL_DEBITUR_INDIVIDU+" WHERE "
                    + PstDebitur.fieldNames[PstDebitur.FLD_PERIODE_ID] + " = '" + periode_id+"'"
                    + " AND " + PstDebitur.fieldNames[PstDebitur.FLD_CIF] + " = '" + cif+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getInt("STATUS_OPERASI_DATA");
            }
            rs.close();
        } catch (Exception e) {
            //System.out.println("err : " + e.toString());
    ...
    public static int getOperasiDataFasilitasKredit(String rekening,long periode_id) {
        DBResultSet dbrs = null;
        int result = -1;
        try {
            String sql = "SELECT * FROM "+PstKredit.TBL_KREDIT+" WHERE "
                    + PstKredit.fieldNames[PstKredit.FLD_PERIODE_ID] + " = '" + periode_id+"'"
                    + " AND " + PstKredit.fieldNames[PstKredit.FLD_NO_REKENING] + " = '" + rekening+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getInt("STATUS_OPERASI_DATA");
    ...
    public static int getOperasiDataFasilitasBankGaransi(String rekening,long periode_id) {
        DBResultSet dbrs = null;
        int result = -1;
        try {
            String sql = "SELECT * FROM "+PstBankGaransi.TBL_BANK_GARANSI+" WHERE "
                    + PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID] + " = '" + periode_id+"'"
                    + " AND " + PstBankGaransi.fieldNames[PstBankGaransi.FLD_NO_REKENING] + " = '" + rekening+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                result = rs.getInt("STATUS_OPERASI_DATA");
