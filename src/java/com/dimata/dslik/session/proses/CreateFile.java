...
public class CreateFile {
    ...
    public static String sentDataToDilimited(FileSent fileSent, OutletConnection outletConnection) {
        ...
        try {
            ...
            String periodeIn="";
            try{
                Periode periodex = new Periode();
                Date datelastmonth = new Date();
                datelastmonth.setMonth(datelastmonth.getMonth()-2);
                Vector listPeriodex = PstPeriode.list(0, 0, ""+PstPeriode.fieldNames[PstPeriode.FLD_TGL_AWAL]+" BETWEEN '2017-03-01' AND '"+Formater.formatDate(datelastmonth,"yyyy-MM-dd")+"'", "");
                if(listPeriodex != null){
                    if(listPeriodex.size()>0){
                        for (int p = 0; p < listPeriodex.size(); p++) {
                            periodex = (Periode) listPeriodex.get(p);
                            if(p==0){
                                periodeIn=periodeIn+"'"+periodex.getOID()+"'";
                            }else{
                                periodeIn=periodeIn+",'"+periodex.getOID()+"'";
                            }
                        }
                ...
                case Configurasi.FLD_SEGMENT_DEBITUR_PERORANGAN:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            where = where + " dslik_debitur.KODE_JENIS_NSB=1 AND dslik_debitur."+PstDebiturIndividu.fieldNames[PstDebiturIndividu.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'" ;
                            
                            if(fileSent.isUpdateOnly()){
//                                where = where +" AND dslik_debitur.CIF NOT IN ("
//                                        + "SELECT TRIM(CIF) AS CIF FROM (" +
//                                            "	SELECT DISTINCT CIF " +
//                                            "	FROM dslik_kredit WHERE KODE_KONDISI IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	AND cif NOT IN (" +
//                                            "	SELECT DISTINCT CIF FROM dslik_kredit WHERE kode_kondisi NOT IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	)" +
//                                            "	UNION ALL" +
//                                            "	SELECT DISTINCT CIF " +
//                                            "	FROM dslik_bank_garansi WHERE KODE_KONDISI IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	AND CIF NOT IN (" +
//                                            "	SELECT DISTINCT CIF FROM dslik_bank_garansi WHERE kode_kondisi NOT IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	)" +
//                                            ") AS ciflunas"
//                                        + ")";
                            }
                            
                            vDebiturIndividu = PstDebiturIndividu.listJoinParentCabang(0, 0, "" + where, "", fileSent.isUpdateOnly(), fileSent.getPeriodeId(), fileSent.isActiveOnly());
                            if (vDebiturIndividu.size() > 0) {
                                ...
                                pw.print("" + vDebiturIndividu.size() + "");//7
                                pw.println();
                                ...
                                for (int p = 0; p < vDebiturIndividu.size(); p++) {
                                    try {
                                        ...
                                        pw.print("" + SessGetContentDataOjk.getContentDataKecamatan(debiturIndividu.getKecamatan(), debiturIndividu.getKodeKab()) + "|");//14
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        if(debiturIndividu.getCif().equals("00333270")){
                                            System.out.print("xxx");
                                        }
                                        
                                        boolean prevDebitur = PstDebiturIndividu.checkDataDebiturPrevPeriode(prevPeriodeId,debiturIndividu.getCif(),periodeIn);
                                        
                                        if(prevDebitur){
                                            debiturIndividu.setStatusOperasiData(1);
                                        }else{
                                            debiturIndividu.setStatusOperasiData(0);
                                        }
                                        
                                        if(fileSent.isCreateInitial()){
                                            debiturIndividu.setStatusOperasiData(0);
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusDebiturPerOrangan("Proses Delimited Text : "+totalProses+"/"+vDebiturIndividu.size());
                                    } catch (Exception exc) {
...
//                                        + ")";
                            ...
                            vBadanUsaha = PstDebiturBdnUsaha.listJoinParentCabang(0, 0, "" + where, "", fileSent.isUpdateOnly(), fileSent.getPeriodeId(), fileSent.isActiveOnly());
                            if (vBadanUsaha.size() > 0) {
                                ...
                                for (int p = 0; p < vBadanUsaha.size(); p++) {
                                    try {
                                        ...
                                        pw.print("" + SessGetContentDataOjk.getContentDataKecamatan(debiturBdnUsaha.getKecamatan(), debiturBdnUsaha.getKodeKab())+ "|");//16
...
//                                        }
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevDebiturBdnUsaha = PstDebiturBdnUsaha.checkDataDebiturBdnUsahaPrevPeriode(prevPeriodeId,debiturBdnUsaha.getCif(),periodeIn);
                                        if(prevDebiturBdnUsaha){
                                            debiturBdnUsaha.setStatusOperasiData(1);
                                        }else{
                                            debiturBdnUsaha.setStatusOperasiData(0);
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusDebiturBadanUsaha("Proses Delimited Text : "+totalProses+"/"+vBadanUsaha.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                ...
                case Configurasi.FLD_SEGMENT_KREDIT:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            if(fileSent.isUpdateOnly() || fileSent.isActiveOnly()){
                                where=where+" AND "+PstKredit.fieldNames[PstKredit.FLD_KODE_KONDISI]+" NOT IN ('01','02','05','06','07','08','09','11','12','14')";
                            }
                            ...
                            if (vKredit.size() > 0) {
                                ...
                                for (int p = 0; p < vKredit.size(); p++) {
                                    try {
                                        ...
                                        pw.print("" + formatter.format(kredit.getProsentaseBunga())+ "|");//22
...
//                                        }
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevKredit = PstKredit.checkDataDebiturBdnUsahaPrevPeriode(prevPeriodeId,kredit.getNoRekening(),kredit.getCif());
                                        if(prevKredit){
                                            kredit.setStatusOperasiData(1);
                                        }else{
                                            kredit.setStatusOperasiData(0);
                                        }
                                        ...
                                        switch (kredit.getStatusOperasiData()) {
                                            ...
                                            case 1:
                                                operasiData="U";
//                                                //cek apakah data debitur nya
//                                                int statusDataDebitur = SessGetContentDataOjk.getOperasiDataDebiturKredit(kredit.getCif(), fileSent.getPeriodeId());
//                                                if(statusDataDebitur==0){
//                                                    operasiData="C";
//                                                }
                                                break;
                                            default:
                                                operasiData="N";
//                                                int statusDataDebiturx = SessGetContentDataOjk.getOperasiDataDebiturKredit(kredit.getCif(), fileSent.getPeriodeId());
//                                                if(statusDataDebiturx==0){
//                                                    operasiData="C";
//                                                }
                                                break;
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusKredit("Proses Delimited Text : "+totalProses+"/"+vKredit.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                            ...
                            if (vKreditJoinAccount.size() > 0) {
                                ...
                                for (int p = 0; p < vKreditJoinAccount.size(); p++) {
                                    try {
                                        ...
                                        if(fileSent.isUpdateOnly()){
                                            kreditJoinAccount.setStatusOperasiData(1);
                                        }
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            String where = "";
                                    ...
                                    where=where + " "+PstSuratBerharga.fieldNames[PstSuratBerharga.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            ...
                            if (vSuratBerharga.size() > 0) {
                                ...
                                for (int p = 0; p < vSuratBerharga.size(); p++) {
                                    try {
                                        ...
                                        if(fileSent.isUpdateOnly()){
                                            suratBerharga.setStatusOperasiData(1);
                                        }
                            ...
                            if (vIrrevocable.size() > 0) {
                                ...
                                for (int p = 0; p < vIrrevocable.size(); p++) {
                                    try {
                                        ...
                                        if(fileSent.isUpdateOnly()){
                                            irrevocableLc.setStatusOperasiData(1);
                                        }
                ...
                case Configurasi.FLD_SEGMENT_BANK_GARANSI:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            where=where + "dslik_bank_garansi."+PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            
                            if(fileSent.isUpdateOnly() || fileSent.isActiveOnly()){
                                where=where+" AND dslik_bank_garansi."+PstBankGaransi.fieldNames[PstBankGaransi.FLD_KODE_KONDISI]+" NOT IN ('01','02','05','06','07','08','09','11','12','14')";
                            }        
                            ...
                            if (vBankGaransi.size() > 0) {
                                ...
                                for (int p = 0; p < vBankGaransi.size(); p++) {
                                    try {
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevBankGaransi = PstBankGaransi.checkDataBankGaransiPrevPeriode(prevPeriodeId,bankGaransi.getNoRekening(),bankGaransi.getCif());
                                        if(prevBankGaransi){
                                            bankGaransi.setStatusOperasiData(1);
                                        }else{
                                            bankGaransi.setStatusOperasiData(0);
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusBankGaransi("Proses Delimited Text : "+totalProses+"/"+vBankGaransi.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
...
//                                        }
                                        ...
                                        if(fileSent.isUpdateOnly()){
                                            fasilitasLain.setStatusOperasiData(1);
                                        }
                ...
                case Configurasi.FLD_SEGMENT_DATA_AGUNAN:
                    ...
                    int totAgunanHapusSebelumLunas = 0;
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            Vector vDataAgunanHapusSebelumLunas = new Vector();
                            if(!fileSent.isCreateInitial()){
                                ...
                                if(fileSent.isUpdateOnly()==false && fileSent.isActiveOnly()==false){
                                    vDataAgunanLunas = PstAgunan.listJoinParentCabangAgunanHapusNew(fileSent.getPeriodeId(), prevPeriodeId);
                                    if(vDataAgunanLunas!=null){
                                        if(vDataAgunanLunas.size()>0){
                                            totAgunanHapus=vDataAgunanLunas.size();
                                        }
                                    }
                                    vDataAgunanHapusSebelumLunas = PstAgunan.listJoinParentCabangAgunanHapusBelumLunas(fileSent.getPeriodeId(), prevPeriodeId);
                                    if(vDataAgunanHapusSebelumLunas!=null){
                                        if(vDataAgunanHapusSebelumLunas.size()>0){
                                            totAgunanHapusSebelumLunas=vDataAgunanHapusSebelumLunas.size();
                                        }
                                    }
                                }
                            }
                            
                            totAgunan=totAgunan+totAgunanHapus+totAgunanHapusSebelumLunas;
                            if (vDataAgunan.size() > 0) {
                                ...
                                for (int p = 0; p < vDataAgunan.size(); p++) {
                                    try {
                                        ...
                                        int paripasu = PstAgunan.checkCountParipasu(fileSent.getPeriodeId(), agunan.getKodeRegisterAgunan());
                                        if(paripasu>1){
                                            agunan.setStatusParipasu("Y");
                                            agunan.setProsentaseParipasu(100/paripasu);
                                        }else{
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevAgunan = PstAgunan.checkDataAgunanPrevPeriode(prevPeriodeId,agunan.getKodeRegisterAgunan(), agunan.getNoRekening(), agunan.getCif());
                                        if(prevAgunan){
                                            agunan.setStatusOperasiData(1);
                                        }else{
                                            agunan.setStatusOperasiData(0);
                                        }
                                        ...
                                        switch (agunan.getStatusOperasiData()) {
                                            ...
                                            case 1:
                                                operasiData="U";
//                                                int statusDataDebitur = SessGetContentDataOjk.getOperasiDataFasilitasKredit(agunan.getNoRekening(), fileSent.getPeriodeId());
//                                                if(statusDataDebitur==0){
//                                                    operasiData="C";
//                                                }else{
//                                                    int statusDataDebiturx = SessGetContentDataOjk.getOperasiDataFasilitasBankGaransi(agunan.getNoRekening(), fileSent.getPeriodeId());
//                                                    if(statusDataDebiturx==0){
//                                                        operasiData="C";
//                                                    }
//                                                }
                                            ...
                                            case 2:
                                                operasiData="N";
//                                                int statusDataDebitury = SessGetContentDataOjk.getOperasiDataFasilitasKredit(agunan.getNoRekening(), fileSent.getPeriodeId());
//                                                if(statusDataDebitury==0){
//                                                    operasiData="C";
//                                                }else{
//                                                    int statusDataDebiturx = SessGetContentDataOjk.getOperasiDataFasilitasBankGaransi(agunan.getNoRekening(), fileSent.getPeriodeId());
//                                                    if(statusDataDebiturx==0){
...
//                                                }
                                ...
                                if(fileSent.isUpdateOnly()==false && fileSent.isActiveOnly()==false){
                                    ...
                                    for (int h = 0; h < vDataAgunanHapusSebelumLunas.size(); h++) {
                                        try {
                                            Agunan agunan = (Agunan) vDataAgunanHapusSebelumLunas.get(h);
                                            totalProses=totalProses+1;                                             
                                            pw.print("" + agunan.getFlagDetail() + "|");//1
                                            pw.print("" + agunan.getKodeRegisterAgunan()+ "|");//2
                                            pw.print("" + agunan.getNoRekening()+ "|");//3
                ...
                case Configurasi.FLD_SEGMENT_PENJAMIN:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            where=where + " dslik_penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"' AND dslik_kredit.KODE_KONDISI!='02'";
                            
                            if(fileSent.isUpdateOnly()){
//                                where = where +" AND dslik_penjamin.CIF NOT IN ("
//                                        + "SELECT TRIM(CIF) AS CIF FROM (" +
//                                            "	SELECT DISTINCT CIF " +
//                                            "	FROM dslik_kredit WHERE KODE_KONDISI IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	AND cif NOT IN (" +
...
//                                        + ")";
                            ...
                            vPenjamin = PstPenjamin.listJoinParentCabang(0, 0, "" + where, "dslik_penjamin."+PstPenjamin.fieldNames[PstPenjamin.FLD_NO_ID_PENJAMIN], fileSent.getPeriodeId(), fileSent.isUpdateOnly(),fileSent.isActiveOnly());
                            if (vPenjamin.size() > 0) {
                                ...
                                for (int p = 0; p < vPenjamin.size(); p++) {
                                    try {
                                        ...
                                        pw.print("" +penjamin.getKodeJenisSegmentFasilitas()+"|");
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevPenjamin = PstPenjamin.checkDataPenjaminPrevPeriode(prevPeriodeId,penjamin.getNoIdPenjamin(), penjamin.getNoRekening(), penjamin.getCif());
                                        if(prevPenjamin){
                                            penjamin.setStatusOperasiData(1);
                                        }else{
                                            penjamin.setStatusOperasiData(0);
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusPenjamin("Proses Delimited Text : "+totalProses+"/"+vPenjamin.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                ...
                case Configurasi.FLD_SEGMENT_PENGURUS_PEMILIK:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            where=where + " dslik_pengurus_atau_pemilik."+PstPengurusAtauPemilik.fieldNames[PstPengurusAtauPemilik.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            
                            if(fileSent.isUpdateOnly()){
//                                where = where +" AND dslik_pengurus_atau_pemilik.CIF NOT IN ("
//                                        + "SELECT TRIM(CIF) AS CIF FROM (" +
//                                            "	SELECT DISTINCT CIF " +
//                                            "	FROM dslik_kredit WHERE KODE_KONDISI IN ('01','02','05','06','07','08','09','11','12','14') AND periode_id='"+fileSent.getPeriodeId()+"'" +
//                                            "	AND cif NOT IN (" +
...
//                                        + ")";
                            ...
                            vPengurusPemilik = PstPengurusAtauPemilik.listJoinParentCabang(0, 0, "" + where, "",fileSent.isUpdateOnly(), fileSent.getPeriodeId(), fileSent.isActiveOnly());
                                ...
                                for (int p = 0; p < vPengurusPemilik.size(); p++) {
                                    try {
                                        ...
                                        try{
                                            ...
                                            if(pengurusPemilik.getPangsaKepemilikan().equals("NULL")){
                                                pw.print("0|");//11
                                            }else{
                                                pw.print("" + pengurusPemilik.getPangsaKepemilikan().replace(".", ",") + "|");//11
                                            }
                                        }catch(Exception ex){
                                            pw.print("0|");//11
                                        }
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevPengurusPemilik = PstPengurusAtauPemilik.checkDataPengurusPemilikPrevPeriode(prevPeriodeId,pengurusPemilik.getNoIdentitas(),pengurusPemilik.getCif());
                                        if(prevPengurusPemilik){
                                            pengurusPemilik.setStatusOperasiData(1);
                                        }else{
                                            pengurusPemilik.setStatusOperasiData(0);
                                        }
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusPengurusPemilik("Proses Delimited Text : "+totalProses+"/"+vPengurusPemilik.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                            ...
                            if (vLaporanKeuanganDebitur.size() > 0) {
                                ...
                                for (int p = 0; p < vLaporanKeuanganDebitur.size(); p++) {
                                    try {
                                        ...
                                        try{
                                            pw.print("" +Formater.formatDate(laporanKeuanganDebitur.getPosisiLaporanKeuanganTahunan() ,"yyyyMM")+ "|");//10
                                        }catch(Exception es){
                                            pw.print(" |");//10
                                        }
                                        ...
                                        //cek apakah ada data ini di periode sebelumnya?
                                        boolean prevLapKeuangan = PstLaporanKeuanganDebitur.checkDataLaporanKeuanganPrevPeriode(prevPeriodeId,laporanKeuanganDebitur.getCif());
                                        if(prevLapKeuangan){
                                            laporanKeuanganDebitur.setStatusOperasiData(1);
                                        }else{
                                            laporanKeuanganDebitur.setStatusOperasiData(0);
                                        }
...
//                            }
                            ...
                            //hapus data dlu
                            try{
                                long deletedatasama = MoveDataCrosPeriode.deleteDataIntialInPeriode(fileSent.getPeriodeId());
                            }catch(Exception ex){
                            }
                            ...
                            int totKreditSummaryLunas = 0;
                            int totBankGaransiSummaryLunas=0;
                            if(fileSent.isCreateInitial()){
                                 totBankGaransiSummaryLunas =PstBankGaransi.getCountLunas("(NO_REKENING, CIF) NOT IN (SELECT NO_REKENING, CIF FROM dslik_bank_garansi WHERE periode_id='"+fileSent.getPeriodeId()+"')");
                                 totBankGaransiSummaryLunas = PstKredit.getCountLunas(" (NO_REKENING, CIF) NOT IN (SELECT NO_REKENING, CIF FROM dslik_kredit WHERE periode_id='"+fileSent.getPeriodeId()+"')");
                            }
                            ...
                            //total summary
                            int totalSummary = totKreditSummary+totBankGaransiSummary+totKreditSummaryLunas+totBankGaransiSummaryLunas;
                            ...
                            if (vSummaryFasilitas.size() > 0) {
                                ...
                                for (int p = 0; p < vSummaryFasilitas.size(); p++) {
                                    try {
                                        ...
                                        Date dateAA = sdfTwo.parse("2016-04-30");
                                        Date dateMM = sdfTwo.parse("2016-05-31");
                                        Date dateJJ = sdfTwo.parse("2016-06-30");
                                        Date dateJLL = sdfTwo.parse("2016-07-31");
                                        Date dateAGG = sdfTwo.parse("2016-08-31");
                                        Date dateSS = sdfTwo.parse("2016-09-30");
                                        Date dateOO = sdfTwo.parse("2016-10-31");
                                        Date dateNN = sdfTwo.parse("2016-11-30");
                                        Date dateDD = sdfTwo.parse("2016-12-31");
                                        Date dateJANN = sdfTwo.parse("2017-01-31");
                                        Date dateFF = sdfTwo.parse("2017-02-28");
                                        Date dateMRR = sdfTwo.parse("2017-03-31");
                                        
                                        
                                        if(summaryFasilitas.getKodeJenisFasilitas().equals("F01")){
                                            if(summaryFasilitas.getNoRekening().equals("0120539021101")){
                                                System.out.println("com.dimata.dslik.session.proses.CreateFile.sentDataToDilimited()");
                                            }
                                            
                                            xxx = MoveDataCrosPeriode.tanggalAwalKredit(periodeId, summaryFasilitas.getNoRekening(), summaryFasilitas.getCif(),fileSent);
                                            cekstatuslunas = MoveDataCrosPeriode.tanggalKondisi(periodeId, summaryFasilitas.getNoRekening(), summaryFasilitas.getCif(),fileSent);
                                            if(xxx!=null){
                                                cekcek=cekcek+1;
                                                if(xxx.compareTo(dateA)>0 || dateA.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    april = true;
                                                }else{
                                                    if(cekstatuslunas!=null){
                                                        if(dateAA.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            april = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateM)>0 || dateM.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateMM.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            mei = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateJ)>0 || dateJ.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateJJ.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            juni = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateJL)>0 || dateJL.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateJLL.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            july = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateAG)>0 || dateAG.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateAGG.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            agustus = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateS)>0 || dateS.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateSS.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            september = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateO)>0 || dateO.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateOO.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            oktober = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateN)>0 || dateN.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateNN.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            november = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateD)>0 || dateD.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateDD.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            desember = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateJAN)>0 || dateJAN.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateJANN.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            januari = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateF)>0 || dateF.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateFF.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            februari = true;
                                                        }
                                                    }
                                                }
                                                ...
                                                if(xxx.compareTo(dateMR)>0 || dateMR.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    ...
                                                    if(cekstatuslunas!=null){
                                                        if(dateMRR.compareTo(sdfTwo.parse(cekstatuslunas.toString()))==0){
                                                            maret = true;
                                                        }
                                                    }
                                                }
                                            ...
                                            xxx = MoveDataCrosPeriode.tanggalAwalBankGaransi(periodeId, summaryFasilitas.getNoRekening(),summaryFasilitas.getCif(),fileSent);
                                            cekstatuslunas = MoveDataCrosPeriode.tanggalKondisiBankGaransi(periodeId, summaryFasilitas.getNoRekening(), summaryFasilitas.getCif(),fileSent);
                                            if(xxx!=null){
                                                cekcek=cekcek+1;
                                                if(xxx.compareTo(dateA)>0 || dateA.compareTo(fileSent.getEndPeriodeDate())==0){
                                                    april = true;
                                        ...
                                        if(insertData){
                                            ...
                                            ManagerDilimitedTextSummary.setStatusDilimitedTextSummary("Proses Delimited Text : masuk : "+cekcek+"/ proses "+countDataSummary+"/"+totalSummary);
    ...
    public static String sentDataToDilimitedIntial(FileSent fileSent, OutletConnection outletConnection) {
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            String where = "";
                                    ...
                                    where = where + " KODE_JENIS_NSB=1 " ;
                            vDebiturIndividu = PstDebiturIndividu.listJoinParentCabangIntial(0, 0, "" + where, "");
                            if (vDebiturIndividu.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_DEBITUR_PERORANGAN] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
                                ...
                                for (int p = 0; p < vDebiturIndividu.size(); p++) {
                                    try {
                                        ...
                                        String operasiData="C";
                                        pw.print("" +operasiData+ "");//39
                                        
                                        pw.println();
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusDebiturPerOrangan("Proses Delimited Text Initial Debitur Individu : "+totalProses+"/"+ vDebiturIndividu.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            String where = "";
                                   ...
                                   where=where + "KODE_JENIS_NSB !=1" ;
                            vBadanUsaha = PstDebiturBdnUsaha.listJoinParentCabangInitial(0, 0, "" + where, "");
                            if (vBadanUsaha.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_DEBITUR_BADAN_USAHA] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            String where = "";
                                    ...
                                    where=where;//+""+PstKredit.fieldNames[PstKredit.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            vKredit = PstKredit.listParentCabangIntial(0, 0, "" + where, "");
                            if (vKredit.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_KREDIT] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
                ...
                case Configurasi.FLD_SEGMENT_BANK_GARANSI:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            vBankGaransi = PstBankGaransi.listJoinParentCabangInitial(0, 0, "" + where, "");
                            if (vBankGaransi.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_BANK_GARANSI] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
                ...
                case Configurasi.FLD_SEGMENT_DATA_AGUNAN:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            //+ " dslik_cabang_bank.PARENT_CODE" + "='" + cabangBank.getKodeCabang() + "' AND "
                            where=where;// + ""+PstAgunan.fieldNames[PstAgunan.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            vDataAgunan = PstAgunan.listJoinParentCabangIntial(0, 0, "" + where, "");
                            ...
                            if (vDataAgunan.size() > 0) {
                                ...
                                pw.print("" + fileSent.getBulan() + "|");//4
                                pw.print("" + Configurasi.valSegment[Configurasi.FLD_SEGMENT_DATA_AGUNAN] + "|");//5
                                pw.print("" + vDataAgunan.size()+ "|");//6
                                pw.print("" + vDataAgunan.size() + "");//7
                                pw.println();
                                //data bulan pelaporan
                                for (int p = 0; p < vDataAgunan.size(); p++) {
                                    try {
                                        ...
                                        if(paripasu>1){
                                            agunan.setStatusParipasu("Y");
                                            if(agunan.getProsentaseParipasu()==0){
                                                agunan.setProsentaseParipasu(100/paripasu);
                                            }
                                        }else{
                                            agunan.setStatusParipasu("T");
                                        ...
                                        //isi
                                        //buatkan file nya
                                        ManagerDilimitedText.setStatusDataAgunan("Proses Delimited Text : "+totalProses+"/"+vDataAgunan.size());
                                    } catch (Exception exc) {
                                        System.out.println("ini eornya" + exc);
                                    }
                                }
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            String where = "";
                                    ...
                                    where=where + " "+PstPenjamin.fieldNames[PstPenjamin.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'";
                            vPenjamin = PstPenjamin.listJoinParentCabangInitial(0, 0, "" + where, "");
                            if (vPenjamin.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_PENJAMIN] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
                ...
                case Configurasi.FLD_SEGMENT_PENGURUS_PEMILIK:
                    ...
                    if (vCabang.size() > 0) {
                        for (int i = 0; i < vCabang.size(); i++) {
                            ...
                            vPengurusPemilik = PstPengurusAtauPemilik.listJoinParentCabangInitial(0, 0, "" + where, "");
                            if (vPengurusPemilik.size() > 0) {
                                namaFile = "" + outletConnection.getJenisLJK() + "." + outletConnection.getKodeLJK() + "." + fileSent.getTahun() + "." + fileSent.getBulan() + "." + Configurasi.valSegment[Configurasi.FLD_SEGMENT_PENGURUS_PEMILIK] + "." + noCabang;
                                patchFle = fileSent.getLocation() + System.getProperty("file.separator") + "" + namaFile + ".txt";
                                pw = new PrintWriter(patchFle);
...
//                            //dapet kan total summary kredit dan bank garansi
//                            int totKreditSummary = PstKredit.getCount(PstKredit.fieldNames[PstKredit.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'");
//                            int totKreditSummaryLunas =PstKredit.getCountLunas(" (NO_REKENING, CIF) NOT IN (SELECT NO_REKENING, CIF FROM dslik_kredit WHERE periode_id='"+fileSent.getPeriodeId()+"')");
//                            int totBankGaransiSummary = PstBankGaransi.getCount(PstBankGaransi.fieldNames[PstBankGaransi.FLD_PERIODE_ID]+"='"+fileSent.getPeriodeId()+"'");
//                            int totBankGaransiSummaryLunas =PstBankGaransi.getCountLunas(" (NO_REKENING, CIF) NOT IN (SELECT NO_REKENING, CIF FROM dslik_bank_garansi WHERE periode_id='"+fileSent.getPeriodeId()+"')");
//                            
//                            //total summary
//                            int totalSummary = totKreditSummary+totBankGaransiSummary+totKreditSummaryLunas+totBankGaransiSummaryLunas;
//                            
