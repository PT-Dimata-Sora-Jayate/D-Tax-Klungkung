            ...
            case Command.SAVE:
                ...
                if (entBankGaransi.getOID() == 0) {
                    ...
                    try {
                        ...
                        BankGaransi entBankGaransiVal =  new BankGaransi();
                        if(!entBankGaransi.getCif().equals("")){
                            entBankGaransi = pstBankGaransi.updateExcObj(this.entBankGaransi);
                            msgString = FRMMessage.getMessage(FRMMessage.MSG_UPDATED);
                            
                            try{
                                entBankGaransiVal = (BankGaransi) entBankGaransiVal.getValidasiDetail(this.entBankGaransi);
                                if(entBankGaransiVal.isIsNotValid()==true){
                                    if(entBankGaransiVal.getOID()!=0){
                                        long updateData = PstBankGaransi.updateStatusDataNotComplate(entBankGaransi.getOID());
                                    }
                                }
                            }catch(Exception es){
                            }
