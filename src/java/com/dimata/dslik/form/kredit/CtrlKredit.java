            ...
            case Command.SAVE:
                ...
                if (entKredit.getOID() == 0) {
                    ...
                    try {
                        ...
                        Kredit kreditVal = new Kredit();
                        if(!entKredit.getCif().equals("")){
                            entKredit = pstKredit.updateExcObj(this.entKredit);
                            try{
                                kreditVal = (Kredit) kreditVal.getValidasiDetail(this.entKredit);
                                if(kreditVal.isIsNotValid()==true){
                                    if(entKredit.getOID()!=0){
                                        long updateData = PstKredit.updateStatusDataNotComplate(entKredit.getOID());
                                    }
                                }
                            }catch(Exception es){
                            }
