            ...
            case Command.SAVE:
                ...
                if (entDebiturIndividu.getOID() == 0) {
                    ...
                    try {
                        ...
                        DebiturIndividu entDebiturIndividuValidasi =  new DebiturIndividu();
                        if(!entDebiturIndividu.getCif().equals("")){
                            entDebiturIndividu = pstDebiturIndividu.updateExcObj(this.entDebiturIndividu);
                            
                            try{
                                entDebiturIndividuValidasi = (DebiturIndividu) entDebiturIndividuValidasi.getValidasiDetail(this.entDebiturIndividu);
                                if(entDebiturIndividuValidasi.isIsNotValid()==true){
                                    if(entDebiturIndividu.getOID()!=0){
                                        long updateData = PstDebiturBdnUsaha.updateStatusDataNotComplate(entDebiturIndividu.getOID());
                                    }
                                }
                            }catch(Exception es){
                            }
