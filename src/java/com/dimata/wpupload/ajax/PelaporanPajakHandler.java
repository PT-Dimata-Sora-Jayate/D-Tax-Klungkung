...
public class PelaporanPajakHandler extends HttpServlet {
    ...
    public JSONObject getDataCompareESPTPD(int total, HttpServletRequest request,String user) throws JSONException{
        ...
        for (int i =0 ; i<=listCompareESPTPD.size()-1;i++){
            ...
            if(AppSetting.TYPE_APP_BACKOFFICE==AppSetting.APP_PHRH){
                 ja.put(""+compareEsptpd.getKodeId());
            }else{
                 ja.put(""+count);
            }
            ...
            ja.put("<a target=\"_blank\" href='"+approot+"/wpupload/document/"+compareEsptpd.getCompareENpwpd()+"/"+compareEsptpd.getNamaDocumentUpload()+"'>"+compareEsptpd.getNamaDocumentUpload()+"</a>");
            
            String id = "";
            if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_PHRH){
                id = Long.toString(compareEsptpd.getKodeId());
            }else{
                id = compareEsptpd.getCompareENpwpd().replace(".", "");
            }
    ...
    public JSONObject getDataViewAllSimpatda(int total, HttpServletRequest request,String user) throws JSONException{
        ...
        for (int i =0 ; i<=listViewAllSimpatda.size()-1;i++){
            ...
            boolean isEsptpd = PstESPTPD.checkESPTDEXIST("NPWPD='"+viewAllSimpatda.getNpwpd()+" and MASA_PAJAK='"+indoMonth[Integer.parseInt(viewAllSimpatda.getMasaPajak())-1]+"' and TAHUN_PAJAK='"+viewAllSimpatda.getTahunPajak()+"'");
            if(isEsptpd){
                ja.put("Input Online");
            }else{
