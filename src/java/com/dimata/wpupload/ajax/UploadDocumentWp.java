...
import com.dimata.common.session.folder.FolderFile;
                    ...
                    if (!item.isFormField()) {
                        ...
                        if(typeFile.equals(".jpg") || typeFile.equals(".xlsx") || typeFile.equals(".jpeg") || typeFile.equals(".xls") || typeFile.equals(".pdf")){
                            if(item5.equals("SUCCESS")){
                                //cek apakah folder sudah ada atau tidak
                                FolderFile folderFile = new FolderFile();
                                boolean checkFolderIsExist = folderFile.checkDirectori(AppSetting.WP_UPLOAD_LOCATION_FILE,"/"+npwpd);
                                //jika folder tidak ada,
                                if(!checkFolderIsExist){
                                    //buatkan folder sesuai dengan npwpd
                                    folderFile.create(AppSetting.WP_UPLOAD_LOCATION_FILE, "/"+npwpd);
                                }
                                //lokasi baru
                                String pathFolder= AppSetting.WP_UPLOAD_LOCATION_FILE+"/"+npwpd;
                                
                                item.write(new File(pathFolder + File.separator +item3+"_"+item2+item1+"_"+name));
