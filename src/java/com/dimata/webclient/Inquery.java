...
//    }
    ...
    public Vector InqueryPHR(InqueryProses inqueryProses) {
        ...
        if (AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_OPEN_PHR) {
            //cek list semua invoice di 
            String whereClause = ""; 
            if(!inqueryProses.getsNoId().equals("")){
                    whereClause = " WHERE npwpd='"+inqueryProses.getsNoId()+"' ";
            }
            
            if(!inqueryProses.getsNoInvoice().equals("")){
                if(whereClause.equals("")){
                        whereClause = "  WHERE sNoId='"+inqueryProses.getsNoInvoice()+"' ";
                }else{
                        whereClause = " AND sNoId='"+inqueryProses.getsNoInvoice()+"' ";
                }
            }else{
                if(whereClause.equals("")){
                        whereClause = "  WHERE sNoId='"+inqueryProses.getsNoInvoice()+"' ";
            ...
            Vector listTagihanPhr = SessSimpatda.getListSimpatdaOpenPhr(whereClause);
            for (int i = 0; i < listTagihanPhr.size(); i++) {
                Simpatda simpatda = (Simpatda) listTagihanPhr.get(i);
                ...
                try {
                    ...
                    for (int k = 0; k < list.getLength(); k++) {
                        ...
                        String childsts_tglawal = "TanggalAwal";
                        Node nodests_tglawal = (Node) tableNode.getElementsByTagName(childsts_tglawal).item(0);
                        String tglAwal = nodests_tglawal.getTextContent();
                        
                        String childsts_tglakhir = "TanggalAkhir";
                        Node nodests_akhir = (Node) tableNode.getElementsByTagName(childsts_tglakhir).item(0);
                        String tglAkhir = nodests_akhir.getTextContent();
                        
                        String childsts_keterangan = "Keterangan";
                        Node nodests_keterangan = (Node) tableNode.getElementsByTagName(childsts_keterangan).item(0);
                        String keterangan = nodests_keterangan.getTextContent();
                        
                        String childsts_npwpd = "NPWPD";
                        Node nodests_npwpd = (Node) tableNode.getElementsByTagName(childsts_npwpd).item(0);
                        String npwpd = nodests_npwpd.getTextContent();
                        ...
                        tagihan.setTanggalAwal(tglAwal);
                        tagihan.setTanggalAkhir(tglAkhir);
                        tagihan.setNpwpd(npwpd);
                        tagihan.setKetTagihanLain(keterangan);
                        listTagihan.add(tagihan);
                    }
                    setLogErorInquery(""+getLogErorInquery()+" Berhasil ");
                } catch (Exception ex) {
                    ex.printStackTrace();
                    setLogErorInquery("  Gagal Cek Koneksi, Respon : "+raw_respon+" Exception : "+ex);
                }
            }
            ...
            String whereClause = ""; 
            if(!inqueryProses.getsNoId().equals("")){
                    whereClause = " WHERE npwpd_search='"+inqueryProses.getsNoId()+"' ";
            }
            
            if(!inqueryProses.getsNoInvoice().equals("")){
                if(whereClause.equals("")){
                        whereClause = "  WHERE kode_id='"+inqueryProses.getsNoInvoice()+"' ";
                }else{
                        whereClause = " AND kode_id='"+inqueryProses.getsNoInvoice()+"' ";
                }
            }else{
                if(whereClause.equals("")){
                        whereClause = "  WHERE kode_id='"+inqueryProses.getsNoInvoice()+"' ";
            ...
            Vector listTagihanPhr = SessSimpatda.getListSimpatdaPhrHLengkap(whereClause);
            for (int i = 0; i < listTagihanPhr.size(); i++) {
                Simpatda simpatda = (Simpatda) listTagihanPhr.get(i);
                inqueryProses.setsNoId(simpatda.getId());
                String raw_respon ="";
                try {
                    ...
                    for (int k = 0; k < list.getLength(); k++) {
                        ...
                        String childPokok = "TagihanPajak";
                        Node nodePokok = (Node) tableNode.getElementsByTagName(childPokok).item(0);
                        String Pokok = nodePokok.getTextContent();

                        String childDenda = "TagihanDenda";
                        Node nodeDenda = (Node) tableNode.getElementsByTagName(childDenda).item(0);
                        String Denda = nodeDenda.getTextContent();
                        
                        String childTagihanAdmn = "TagihanAdmin";
                        Node nodeTagAdmin = (Node) tableNode.getElementsByTagName(childTagihanAdmn).item(0);
                        String TagihanAdmin = nodeTagAdmin.getTextContent();

                        String childsts_bayar = "sts_bayar";
                        Node nodests_bayar = (Node) tableNode.getElementsByTagName(childsts_bayar).item(0);
                        String sts_bayar = nodests_bayar.getTextContent();
                        
                        String childsts_JenisUsaha = "JenisUsaha";
                        Node nodests_JenisUsaha = (Node) tableNode.getElementsByTagName(childsts_JenisUsaha).item(0);
                        String jenisUsaha = nodests_JenisUsaha.getTextContent();
                        
                        String childsts_npwpd = "NPWPD";
                        Node nodests_npwpd = (Node) tableNode.getElementsByTagName(childsts_npwpd).item(0);
                        String npwpd = nodests_npwpd.getTextContent();
                        
                        String childsts_jatuhTempo = "TanggalJatuhTempo";
                        Node nodests_jatuhTempo = (Node) tableNode.getElementsByTagName(childsts_jatuhTempo).item(0);
                        String tglJatuhTempo = nodests_jatuhTempo.getTextContent();
                        
                        String childsts_waktu = "Waktu";
                        Node nodests_waktu = (Node) tableNode.getElementsByTagName(childsts_waktu).item(0);
                        String waktu = nodests_waktu.getTextContent();
                        ...
                        tagihan.setStsBayar(sts_bayar);//1
                        tagihan.setNpwpd(npwpd);
                        tagihan.setJenisUsaha(jenisUsaha);
                        tagihan.setTagihanAdmin(TagihanAdmin);
                        tagihan.setWaktu(waktu);
                        
                        tagihan.setTanggalJatuhTempo(tglJatuhTempo);
	...
	public Vector InqueryBPHTBIprotax(InqueryProses inqueryProses) {
        String resp_code = new String();
        String resp_message = new String();
        Vector listTagihan = new Vector();
        try {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                String childPPAT = "PPAT";
                Node nodePPAT = (Node) tableNode.getElementsByTagName(childPPAT).item(0);
                String PPAT = nodePPAT.getTextContent();
