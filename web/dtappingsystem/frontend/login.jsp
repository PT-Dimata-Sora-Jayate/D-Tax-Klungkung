...
<html>
    <head>
    <meta charset="UTF-8">
    <title>TAPPING | LOGIN</title>
    <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
    <%@include file="../template-component/css-component.jsp" %>
    <style>
        html { 
            background: url(../../styles/bootstrap/css/bgcovertapping.jpg)  no-repeat center center fixed; 
    ...
    <body style="background: none;">
        <div class="login-logo" style="margin: 1% auto;">
            <img class="logos" src="../../imgcompany/dimata_system_logo.png" width="200px;">
        </div><!-- /.login-logo -->
        <div >
            <center><h1><font color="#ffffff">INTEGRASI TAPPING SYSTEM</font></h1></center>
        </div>
        <div class="login-box" style="margin: 1% auto;">
          ...
          <form name="frmLogin" action="<%=approot%>/dtappingsystem/frontend/login.jsp" onsubmit="window.status=''" method="post">
	   <input type="hidden" name="command" value="<%=CMD_LOGIN%>">
		...
		if((iCommand==CMD_LOGIN)&&(dologin == SessUserSession.DO_LOGIN_OK)){
		     response.sendRedirect(approot+"/home_tapping.jsp");
		 }
	    %>
		...
		<span class="glyphicon glyphicon-lock form-control-feedback"></span>
	      ...
	      <div class="row">
		<div class="col-xs-12">
		    <button type="submit" class="btn btn-danger btn-block btn-flat" onclick="javascript:cmdLogin()">Sign In</button>
		</div><!-- /.col -->
	      </div>
