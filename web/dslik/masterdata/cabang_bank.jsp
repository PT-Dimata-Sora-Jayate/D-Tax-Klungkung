...
<%@page import="com.dimata.dslik.form.masterdata.FrmCabangBank"%>
...
boolean privAdd = userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_MODUL_MASTER_DATA, AppObjInfo.G2_PROFILE_BANK, AppObjInfo.OBJ_CABANG_BANK, AppObjInfo.COMMAND_ADD);
boolean privUpdate = userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_MODUL_MASTER_DATA, AppObjInfo.G2_PROFILE_BANK, AppObjInfo.OBJ_CABANG_BANK, AppObjInfo.COMMAND_UPDATE);
boolean privDelete = userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_MODUL_MASTER_DATA, AppObjInfo.G2_PROFILE_BANK, AppObjInfo.OBJ_CABANG_BANK, AppObjInfo.COMMAND_DELETE);
boolean privView = userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_MODUL_MASTER_DATA, AppObjInfo.G2_PROFILE_BANK, AppObjInfo.OBJ_CABANG_BANK, AppObjInfo.COMMAND_VIEW);
boolean privDownload = userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_MODUL_MASTER_DATA, AppObjInfo.G2_PROFILE_BANK, AppObjInfo.OBJ_CABANG_BANK, AppObjInfo.COMMAND_DOWNLOAD);

if(iCommand==Command.SUBMIT){
    ProsesTransferDataBank prosesTransferDataBank = new ProsesTransferDataBank();
    int periodeHost = prosesTransferDataBank.getDataCabangBank();
}
%>
<html>
    <head>
        ...
        <script language="JavaScript">
            function cmdDownload(){
                document.cabangbank.command.value="<%=Command.SUBMIT%>";
                document.cabangbank.action="cabang_bank.jsp";
                document.cabangbank.submit();
            }
        </script>
    </head>
    <body class="<%= skin%>">
        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                ...
                <section class="content">
                    ...
                    <%--<form name="cabangbank"  method ="post" action="" role="form">
                                ...
                                <div class='box box-primary'>
                                    ...
                                    <div class='box-footer'>
                                        <%
                                            if(privAdd){
                                                %>
                                                    <button class="btn btn-primary btnaddgeneral" data-oid="0" data-for="showform">
                                                        <i class="fa fa-plus"></i> Add Cabang Bank
                                                    </button>
                                                <%
                                            }
                                            ...
                                            if(privDownload){
                                                %>
                                                    <button class="btn btn-success btndownloadcabang" data-for="country" onClick="javascript:cmdDownload()" >
                                                        <i class="fa fa-download"></i> Download Cabang Bank 
                                                    </button>
                                                <%
                                            }
                                        %>
                ...
                $(document).ready(function () {
                    ...
                    addeditgeneral = function (elementId) {
                        $(elementId).click(function () {
                            ...
                            if (oid != 0) {
                                if (dataFor == 'showform') {
                                    $(".addeditgeneral-title").html("Edit Cabang Bank");
                                }

                            } else {
                                if (dataFor == 'showform') {
                                    $(".addeditgeneral-title").html("Add Cabang Bank");
                                }
                            }
                            ...
                            getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxCabangBank", ".addeditgeneral-body", false, "json");
                        });
                    };
                    ...
                    deletegeneral = function (tableId, elementId) {
                        ...
                        $(tableId).on("click", elementId, function () {
                            ...
                            if (confirm(confirmText)) {
                                ...
                                getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxCabangBank", null, true, "json");
                    ...
                    //FORM HANDLER
              chanelForm = function ()       {
                        validateOptions("#<%=FrmCabangBank.fieldNames[FrmCabangBank.FRM_FIELD_CABANG_ID]%>", 'text', 'has-error', 1, null);
                    }
                    ...
                    function dataTablesOptions(elementIdParent, elementId, servletName, dataFor, callBackDataTables) {
                        ...
                        $("#" + elementId).dataTable({"bDestroy": true,
                            ...
                            "bServerSide":       true,
                            "sAjaxSource": "<%= approot%>/" + servletName + "?command=<%= Command.LIST%>&FRM_FIELD_DATA_FILTER=" + datafilter + "&FRM_FIELD_DATA_FOR=" + dataFor + "&privupdate=" + privUpdate + "&FRM_FLD_APP_ROOT=<%=approot%>&privdelete="+privDelete,
                            aoColumnDefs: [
                                {
                                    bSortable: false,
                                    aTargets: [-1, -2]
                    ...
                    function runDataTables() {
                        dataTablesOptions("#countryElement", "tableCountryElement", "AjaxCabangBank", "list", callBackDataTables);
                    }
