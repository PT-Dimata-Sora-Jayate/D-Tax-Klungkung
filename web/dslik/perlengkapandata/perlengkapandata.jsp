...
%>
<%
    if(runningProsesDebitur || runningProsesKredit && runningProsesBankGaransi || runningProsesAgunan || runningProsesPengurus){
        response.sendRedirect(approot + "/home_slik_transfer_data.jsp");
    }
%>
<html>
  <head>
  ...
  <body class="<%=skin%>">
    ...
    <div id="shownoidentitas" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="box-body addeditgeneral-body">
                                    ...
                                    <div id="identitasTableElement">
                                        <table class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    <td>No.</td>
                                                    <td>No. Identitas</td>                                                    
                                                    <td id="namaIdentitas"></td>                                                                                                  
                                                    <td>Alamat</td>
                                                    <td>Action</td>
                                                </tr>
                                            </thead>
    ...
    <div id="showmasterpenjamin" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="box-body addeditgeneral-body">
                                    ...
                                    <div id="masterPenjaminTableElement">
                                        <table class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    <th>No.</th>
                                                    <th>Nomor Identitas Penjamin</th>
                                                    <th>Jenis Identitas</th>
                                                    <th>Nama Identitas</th>
                                                    <th>Nama Lengkap</th>
                                                    <th>Kode Golongan Penjamin</th>
	...
	$(document).ready(function(){
		...
		$(selector).click(function(){    
			...
			getDataFunction(onDone, onSuccess, approot, command, datasend, target, "#formcontent", false);
                    ...
                    } else if (datafor == "searchidentitas"){
                        modaltitle = "Pencarian Nomor Identitas";
                        if (target == "AjaxPengurusPemilik"){   
                            var search = $('#FRM_FIELD_NO_IDENTITAS').val();
                            $('#namaIdentitas').html("Nama Pengurus");
                            dataTablesOptions("#identitasTableElement", "tableIdentitasDataElement", ""+target+"", ""+datafor+"", null);
                            $('input[type="search"]').val(search).keyup()
                        } else if (target == "AjaxPenjamin") {
                            var search = $('#FRM_FIELD_NO_ID_PENJAMIN').val();
                            $('#namaIdentitas').html("Nama Penjamin");
                            dataTablesOptions("#identitasTableElement", "tableIdentitasDataElement", ""+target+"", ""+datafor+"", null);                            
                            $('#shownoidentitas #identitasTableElement input[type="search"]').val(search).keyup()
                        }
                    } else if (datafor == "listselect"){
                        modaltitle = "Pencarian Master Penjamin";
                        if (target == "AjaxMasterPenjamin") {
                            var search = $('#FRM_FIELD_NO_ID_PENJAMIN').val();                            
                            dataTablesOptions("#masterPenjaminTableElement", "tableMasterPenjaminDataElement", ""+target+"", ""+datafor+"", null);                            
                            $('#showmasterpenjamin #masterPenjaminTableElement input[type="search"]').val(search).keyup()
                        }
                    }                    
            ...
            $('body').on("click",".btn-selectnoidentitas",function(){                    
                var datafor = $(this).data("for");
                var oid = $(this).data("oid");
                var command = $(this).data("command");
                var target = $(this).data("target");
                ...
                var onDone = function(data){
                    if (target == "AjaxPengurusPemilik") {                        
                        $.map(data.RETURN_DATA_PENGURUS, function (item) {
                            //alert(item.nama_pengurus);
                            $('#FRM_FIELD_NO_IDENTITAS').val(item.no_identitas);
                            $('#FRM_FIELD_JENIS_IDENTITAS').val(item.jenis_identitas);
                            $('#FRM_FIELD_NAMA_PENGURUS').val(item.nama_pengurus);
                            $('#FRM_FIELD_JENIS_KELAMIN').val(item.jenis_kelamin);
                            $('#FRM_FIELD_ALAMAT').val(item.alamat);
                            $('#FRM_FIELD_KELURAHAN').val(item.kelurahan);
                            $('#FRM_FIELD_KECAMATAN').val(item.kecamatan);
                            $('#FRM_FIELD_KODE_KABUPATEN').val(item.kodekab);
                        });
                    } else if (target == "AjaxPenjamin") {
                        $.map(data.RETURN_DATA_PENJAMIN, function (item) {
                            //alert(item.nama_sesuai_identitas);
                            $('#FRM_FIELD_NO_ID_PENJAMIN').val(item.no_identitas);
                            $('#FRM_FIELD_NO_REKENING').val(item.no_rekening);
                            //$('#FRM_FIELD_CIF').val(item.cif);
                            $('#FRM_FIELD_KODE_JENIS_SEGMENT_FASILITAS').val(item.kode_jenis_segmen);
                            $('#FRM_FIELD_JENIS_IDENTITAS').val(item.jenis_identitas);
                            $('#FRM_FIELD_NAMA_IDENTITAS').val(item.nama_sesuai_identitas);
                            $('#FRM_FIELD_NAMA_LENGKAP').val(item.nama_lengkap);
                            $('#FRM_FIELD_KODE_GOL_PENJAMIN').val(item.kode_golongan_penjamin);
                            $('#FRM_FIELD_ALAMAT_PENJAMIN').val(item.alamat_penjamin);
                            $('#FRM_FIELD_KETERANGAN').val(item.keterangan);
                        });
                    } else if (target == "AjaxMasterPenjamin") {                        
                        $.map(data.RETURN_DATA_MASTER_PENJAMIN, function (item) {
                            $('#FRM_FIELD_NO_ID_PENJAMIN').val(item.no_identitas);
                            $('#FRM_FIELD_JENIS_IDENTITAS').val(item.jenis_identitas);
                            $('#FRM_FIELD_NAMA_IDENTITAS').val(item.nama_sesuai_identitas);
                            $('#FRM_FIELD_NAMA_LENGKAP').val(item.nama_lengkap);
                ...
                var onSuccess = function(data){
                    if (target == "AjaxMasterPenjamin") {
                        $('#showmasterpenjamin').modal("hide");
                    } else {
                        $('#shownoidentitas').modal("hide");
                    }                    
                };
                getDataFunction(onDone, onSuccess, approot, command, datasend, target, null, false);
            });
            
            $('body').on("change","#FRM_FIELD_NO_REKENING",function(){
                var text = $('#FRM_FIELD_NO_REKENING option:selected').text();
                if (text.includes("F01")) {
                    $('#FRM_FIELD_KODE_JENIS_SEGMENT_FASILITAS').val("F01");
                } else if (text.includes("F05")) {
                    $('#FRM_FIELD_KODE_JENIS_SEGMENT_FASILITAS').val("F05");
                } else {
                    $('#FRM_FIELD_KODE_JENIS_SEGMENT_FASILITAS').val("");
                }
            });
		    ...
		    var onDone = function(data){
                        ...
                        showForm("#btn-showmodalmasterpenjamin","#showmasterpenjamin");
                        modalSetting("#showmasterpenjamin","static",false,false);
                        
                        showForm("#btn-showmodalnoidentitas","#shownoidentitas");
                        modalSetting("#shownoidentitas","static",false,false);
