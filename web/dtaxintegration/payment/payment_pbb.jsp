...
//            rowx.add(df.format(
//                Math.ceil(
//                    Double.parseDouble(payment.getTagihan().replace(".", "").replace(",", ".")) -
//                    Double.parseDouble(payment.getDenda().replace(".", "").replace(",", "."))
//                )
//            ));
//
//            // Row 9
...
//                Double.parseDouble(payment.getDenda().replace(".", "").replace(",", "."))
//            ));
            String dendaString = payment.getDenda();
            if(dendaString.contains(".0")){
                dendaString = dendaString.replace(".0", "");
            } else if (dendaString.contains(".00")){
                dendaString = dendaString.replace(".00", "");
            } 

            if (dendaString.contains(".")){
                dendaString = dendaString.replace(".", "");
            }
            rowx.add(""+FRMHandler.userFormatStringDecimal((Math.ceil(Double.parseDouble(payment.getTagihan()))-Double.parseDouble(dendaString))));//8
            rowx.add(""+FRMHandler.userFormatStringDecimal(Double.parseDouble(dendaString)));//9
    ...
    public String drawListDataPaymentPhrDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
			...
			rowx.add(""+FRMHandler.userFormatStringDecimal((Math.ceil(paymentPbb.getJmlSpptYgDibayar()-paymentPbb.getDendaSppt()))));//8
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    if(AppSetting.SQL_VERSION==AppSetting.DBSVR_ORACLE){
        ...
        if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX || AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX_V2){
            whereClause = " TGL_PEMBAYARAN_SPPT BETWEEN '"+startDate+" 00:00:00' AND '"+startDate+" 23:59:00' AND NO_TRANSAKSI_BYR_SPPT_BANK IS NOT NULL AND NO_TRANSAKSI_BYR_SPPT_BANK <> ''";
        }
    }
