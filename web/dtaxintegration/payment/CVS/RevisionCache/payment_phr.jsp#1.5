...
<%@page import="com.dimata.dtaxintegration.entity.payment.PaymentPhrforPhrH"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PstPaymentPhrforPhrH"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PaymentPhrforOpenPhr"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PstPaymentPhrforOpenPhr"%>
...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPhrBpd(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            double tagihan=0;
            try{
                tagihan=Double.parseDouble(payment.getTagihan());
            }catch(Exception ex){}
            total=total+tagihan;
    ...
    public String drawListDataPaymentPhrDispendaOpenPhr(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            //Simpatda simpatda = (Simpatda) objectClass.get(i);
            PaymentPhrforOpenPhr paymentPhr = (PaymentPhrforOpenPhr) objectClass.get(i);
            count=count+1;
            rowx = new Vector(1, 1);
            
            rowx.add(""+count);//1
            rowx.add(""+paymentPhr.getIdBank());//2
            rowx.add(""+paymentPhr.getNoId());//2
            rowx.add(""+paymentPhr.getNpwpd());//2
            rowx.add(""+paymentPhr.getNama());//3
            rowx.add(""+paymentPhr.getTglTransaksi());//4
            rowx.add(""+paymentPhr.getBulan());//6
            rowx.add(""+paymentPhr.getTahun());//7
            rowx.add(""+FRMHandler.userFormatStringDecimal(paymentPhr.getTagihan()));//8
            rowx.add(""+paymentPhr.getStatusReversal());//8
            total=total+paymentPhr.getTagihan();
    ...
    public String drawListDataPaymentPhrDispendaForPhrH(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            //Simpatda simpatda = (Simpatda) objectClass.get(i);
            PaymentPhrforPhrH paymentPhr = (PaymentPhrforPhrH) objectClass.get(i);
            count=count+1;
            rowx = new Vector(1, 1);
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    if(iCommand==Command.POSTING){
        ...
        switch (AppSetting.TYPE_APP_BACKOFFICE) {
            case AppSetting.APP_OPEN_PHR:
                dtaxIntegration.inputPaymentPHRforOpenPHR(startDate, "");
                break;
            case AppSetting.APP_PHRH:
                String whereClauseNoBukti = PstPaymentPhrforOpenPhr.fieldNames[PstPaymentPhrforOpenPhr.FLD_TGLTRANSAKSI]+" LIKE '%"+startDate+"%'";
                String noBuktiTerakhir = PstPaymentPhrforPhrH.noBuktiAkhir(whereClauseNoBukti);
                dtaxIntegration.inputPaymentPHRforPhrH(startDate, "",""+noBuktiTerakhir);
                break;
            default:
    ...
    switch (AppSetting.TYPE_APP_BACKOFFICE) {
        case AppSetting.APP_OPEN_PHR:
            listReportDetail = echoLaporan.getListPaymentDetailPHRforOpenPhr(laporanPayment);
            break;
        case AppSetting.APP_PHRH:
            listReportDetail = echoLaporan.getListPaymentDetailPHRforPhrH(laporanPayment);
            break;
        default:
    ...
    switch (AppSetting.TYPE_APP_BACKOFFICE) {
        case AppSetting.APP_OPEN_PHR:
            whereClause = PstPaymentPhrforOpenPhr.fieldNames[PstPaymentPhrforOpenPhr.FLD_TGLTRANSAKSI]+" LIKE '%"+startDate+"%'";
            listPaymentPhr = PstPaymentPhrforOpenPhr.list(0, 0, whereClause, "");
            break;
        case AppSetting.APP_PHRH:
            whereClause = PstPaymentPhrforOpenPhr.fieldNames[PstPaymentPhrforOpenPhr.FLD_TGLTRANSAKSI]+" LIKE '%"+startDate+"%'";
            listPaymentPhr = PstPaymentPhrforPhrH.list(0, 0, whereClause, "");
            break;
        default:
                               ...
                               <div class="box">
                                   <div class="box-header">
                                    ...
                                    switch (AppSetting.TYPE_APP_BACKOFFICE) {
                                        case AppSetting.APP_OPEN_PHR:
                                            %>
                                                <%=drawListDataPaymentPhrDispendaOpenPhr(iCommand, listPaymentPhr, oidDriver)%>
                                            <%
                                            break;
                                        case AppSetting.APP_PHRH:
                                            %>
                                                <%=drawListDataPaymentPhrDispendaForPhrH(iCommand, listPaymentPhr, oidDriver)%>
                                            <%
                                            break;
                                        default:
                                            %>
