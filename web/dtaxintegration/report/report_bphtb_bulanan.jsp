...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPBBDispendaHarian(int iCommand, Vector objectClass, long gradeId) {
        ...
        ctrlist.addHeader("NO", "200px");//1
        ctrlist.addHeader("BULAN", "200px");//2
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx = new Vector(1, 1);
            rowx.add(""+count);//1
            rowx.add(""+Formater.formatDate(paymentBphtb.getTglPembayaran(), "MMM-yyyy").toUpperCase());//2
            rowx.add(""+Formater.formatNumber(paymentBphtb.getBphtbSdhBayar(),"#,###"));//3
...
<% 
   ...
   /* EchoLaporanPaymentDetail echoLaporan = new EchoLaporanPaymentDetail();
    ...
    /*data table payment*/
    Vector listPaymentBPHTB = new Vector();
	...
	String whereClauseIprotax =PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]
                  +" BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oEnd+"','YYYY-MM-DD HH24:MI:SS')";
    ...
    /*listPaymentBPHTB = PstPaymentPbbIprotax.listPerBulan(0, 0, whereClauseIprotax, 
            "convert(varchar(7), TGL_PEMBAYARAN_SPPT, 126) ASC",
            "convert(varchar(7), TGL_PEMBAYARAN_SPPT, 126)");*/
	
	listPaymentBPHTB = PstPaymentBphtbIprotax.listPerBulan(0, 0, whereClauseIprotax, 
            ""+PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]+" ASC",
            "TO_CHAR("+PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]+",'Month YYYY')");
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdSearch(){
                ...
                document.driver.action="report_bphtb_bulanan.jsp";
                document.driver.submit();
            }
            
            function cmdPrintExcel(){
                var myWindow = window.open("report_bphtb_excel_bulanan.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_bphtb_bulanan_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
                            ...
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <div class="box-header">
                                    ...
                                    <%=drawListDataPaymentPBBDispendaHarian(iCommand, listPaymentBPHTB, oidDriver)%>
                                </div><!-- /.box -->
                            </div>
                        </div>      
                    </form>
