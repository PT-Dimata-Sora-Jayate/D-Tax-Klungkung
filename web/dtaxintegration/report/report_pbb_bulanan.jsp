...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPhrBpd(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+payment.getStsReversal());
    ...
    public String drawListDataPaymentPBBDispendaHarian(int iCommand, Vector objectClass, long gradeId) {
        ...
        ctrlist.addHeader("POKOK (Rp)", "200px");//3
        ctrlist.addHeader("DENDA (Rp)", "200px");//7
        ctrlist.addHeader("JUMLAH BAYAR (Rp)", "200px");//10
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+count);//1
            rowx.add(""+Formater.formatDate(paymentPbb.getTglPembayaranSppt(), "MMM-yyyy").toUpperCase());//2
...
<% 
   ...
   /* EchoLaporanPaymentDetail echoLaporan = new EchoLaporanPaymentDetail();
	...
	/*String whereClauseIprotax =PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]
                  +" BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oEnd+"','YYYY-MM-DD HH24:MI:SS')";*/
	
	String whereClauseIprotax = PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]
                  +" BETWEEN CAST('"+startDate+" 00:01:00' as datetime)"
                  +" AND CAST('"+startEnd+" 23:59:00' as datetime)";
    
    listPaymentPBB = PstPaymentPbbIprotax.listPerBulan(0, 0, whereClauseIprotax, 
            "convert(varchar(7), TGL_PEMBAYARAN_SPPT, 126) ASC",
            "convert(varchar(7), TGL_PEMBAYARAN_SPPT, 126)");
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                ...
                document.driver.action="report_pbb_bulanan.jsp";
            ...
            function cmdPrintExcel(){
                var myWindow = window.open("report_pbb_excel_bulanan.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_pbb_bulanan_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            <aside class="right-side">                
                ...
                <section class="content">
                    <form name="driver"  method ="post" action="" role="form">
                        ...
                        <input type="hidden" name="menu" value="152">
