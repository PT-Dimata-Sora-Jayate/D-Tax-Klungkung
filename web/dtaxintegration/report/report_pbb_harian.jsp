...
<%@page import="com.dimata.dtaxintegration.entity.payment.PaymentPbbIprotax"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PstPaymentPbbIprotax"%>
...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPBBDispendaHarian(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            PaymentPbbIprotax paymentPbb = (PaymentPbbIprotax) objectClass.get(i);
            count=count+1;
            rowx = new Vector(1, 1);
            double pokok = paymentPbb.getJmlPbbYgDibayar()-paymentPbb.getJmlDendaSppt();
            rowx.add(""+count);//1
            rowx.add(""+Formater.formatDate(paymentPbb.getTglPembayaranSppt(), "dd-MMM-yyyy").toUpperCase());//2
            rowx.add(""+Formater.formatNumber(pokok,"#,###"));//3
            rowx.add(""+Formater.formatNumber(paymentPbb.getJmlDendaSppt(),"#,###"));//4
            rowx.add(""+Formater.formatNumber(paymentPbb.getJmlPbbYgDibayar(),"#,###"));//6
            total=total+paymentPbb.getJmlPbbYgDibayar();
            totalPokok=totalPokok+pokok;
            totalDenda=totalDenda+paymentPbb.getJmlDendaSppt();
        ...
        rowx.add("");//1
        rowx.add("<b>Total</b>");//2
        rowx.add(""+FRMHandler.userFormatStringDecimal(totalPokok));//3
        rowx.add(""+FRMHandler.userFormatStringDecimal(totalDenda));//4
...
<% 
   ...
   /* EchoLaporanPaymentDetail echoLaporan = new EchoLaporanPaymentDetail();
    ...
    /*whereClause = PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_STATUS]+"='0'"
                  +" AND TRUNC("+PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]
                  +") BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oEnd+"','YYYY-MM-DD HH24:MI:SS')";*/
	
	String whereClauseIprotax = " TRUNC("+PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]
                  +") BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oEnd+"','YYYY-MM-DD HH24:MI:SS')";
	
	String whereClauseSqlServer = PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]
                  +" BETWEEN CAST('"+startDate+" 00:00:00' as datetime)"
                  +" AND CAST('"+startEnd+" 23:59:00' as datetime)";
    
    /*listPaymentPBB = PstPaymentPbb.listIpprotax(0, 0, whereClause, 
            ""+PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" ASC GROUP BY " 
            +PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]);*/
	
	/*listPaymentPBB = PstPaymentPbbIprotax.listPaymentPBBDaily(0, 0, whereClauseIprotax, 
			""+PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" ASC",
            "TO_CHAR(TGL_PEMBAYARAN_SPPT,'DD Month YYYY')");*/
	if (AppSetting.SQL_VERSION == AppSetting.DBSVR_ORACLE){
		listPaymentPBB = PstPaymentPbbIprotax.listPaymentPBBDaily(0, 0, whereClauseIprotax, 
				""+PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" ASC",
				"TO_CHAR(TGL_PEMBAYARAN_SPPT,'DD Month YYYY')");
	} else {
		listPaymentPBB = PstPaymentPbbIprotax.listPaymentPBBDaily(0, 0, whereClauseSqlServer, 
				"CAST(TGL_PEMBAYARAN_SPPT AS DATE) ASC",
				"CAST(TGL_PEMBAYARAN_SPPT AS DATE)");
	}
%>
<html>
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                ...
                document.driver.action="report_pbb_harian.jsp";
            ...
            function cmdPrintExcel(){
                var myWindow = window.open("report_pbb_excel_harian.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
             ...
             function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_pbb_harian_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            <aside class="right-side">                
                ...
                <section class="content">
                    <form name="driver"  method ="post" action="" role="form">
                        ...
                        <input type="hidden" name="menu" value="151">
                            ...
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <div class="box-header">
                                    ...
                                    <%=drawListDataPaymentPBBDispendaHarian(iCommand, listPaymentPBB, oidDriver)%>
                                </div><!-- /.box -->
                            </div>
                        </div>      
                    </form>
