...
<%!
    ...
    public String drawListDataPaymentDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+count);//1
            rowx.add(""+paymentBphtb.getNoTransaksiBayarBank());//2
...
<% //proses yang tidak akan ditampilkan hanya diserver side
	...
	whereClause = "TGL_PEMBAYARAN BETWEEN TO_DATE('"+startDate+"','YYYY-MM-DD')"
                  +" AND TO_DATE ('"+startEnd+"','YYYY-MM-DD')";
    listPayment = PstPaymentBphtbIprotax.list(0, 0, whereClause, "");
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                ...
                document.driver.action="report_bphtb.jsp";
            ...
            function cmdPrintExcel(){
                var myWindow = window.open("report_bphtb_excel.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_bphtb_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
