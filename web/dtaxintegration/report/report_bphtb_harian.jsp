...
<%!
    ...
    public String drawListDataPaymentDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+count);//1
            rowx.add(""+Formater.formatDate(paymentBphtb.getTglPembayaran(), "dd-MMM-yyyy").toUpperCase());//2
...
<% //proses yang tidak akan ditampilkan hanya diserver side
	...
	String whereClauseIprotax = " TRUNC("+PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]
	...
	if (AppSetting.SQL_VERSION == AppSetting.DBSVR_ORACLE){
		listPayment = PstPaymentBphtbIprotax.listPaymentBphtbDaily(0, 0, whereClauseIprotax, 
				""+PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]+" ASC",
				"TO_CHAR("+PstPaymentBphtbIprotax.fieldNames[PstPaymentBphtbIprotax.FLD_TGLPEMBAYARAN]+",'DD Month YYYY')");
	} else {
		listPayment = PstPaymentBphtbIprotax.listPaymentBphtbDaily(0, 0, whereClauseIprotax, 
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdPrint(){
                ...
                document.driver.action="report_bphtb_harian.jsp";
            ...
            function cmdPrintExcel(){
                var myWindow = window.open("report_bphtb_harian_excel.jsp?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>", "MsgWindow", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=1500, height=800");
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            function cmdPrintPdf(){
                var myWindow = window.open("<%=printrootx%>.print.report_bphtb_harian_pdf?tanggalStart=<%=oDate%>&tanggalEnd=<%=oEnd%>&exchangeRate=<%=exchangeRate%>&pemotongan=<%=pemotongan%>","",'scrollbars=yes,status=yes,width=750,height=600,resizable=yes');
                if (window.focus) 
                 {
                   myWindow.focus();
                 }
            ...
            <aside class="right-side">                
                ...
                <section class="content">
                    <form name="driver"  method ="post" action="" role="form">
                        ...
                        <input type="hidden" name="menu" value="191">
                            ...
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <div class="box-header">
                                       ...
                                       <% if(userSession.checkPrivG1G2(AppObjInfo.G1_PROSES_INTEGRASI,AppObjInfo.G2_REPORT_BPHTB)){ %>     
                                           <div class="col-xs-2"><label>&nbsp;</label>  
